...
import mfixgui.gui
from mfixgui.project import Keyword, Equation, Project, FloatExp, main
...
class TestParser(unittest.TestCase):
    ...
    def test_parseKeywordLine_float_d(self):
        ...
        expect = [('key', [i + 1], 10.0 if i == 0 else FloatExp(10)) for i in range(6)]
        self._test(line, expect)

    def test_parseKeywordLine_float_e(self):
        line = "key = 10.0 10e 10.e 10E 10.E 10.0E"
        expect = [
            ('key', [i + 1], 10.0 if i == 0 else FloatExp(10.0)) for i in range(6)
        ]
        self._test(line, expect)
    ...
    def test_parseKeywordLine_shorthand_eq_2(self):
        ...
        expect = [
            ('key', [i + 1], 10 if 0 < i < 4 else Equation("2*5")) for i in range(6)
        ]
        self._test(line, expect)
    ...
    def test_parseKeywordLine_shorthand_eq_3(self):
        ...
        expect = [
            ('key', [i + 1], 10 if 0 < i < 4 else Equation("5+5")) for i in range(6)
        ]
        self._test(line, expect)
...
def mfix_gui(mocker, qtbot):
    mfixgui.gui.__version__ = "19.1.3"
    gui = mocker.Mock()
    mocker.patch.object(gui, "message")
    return gui
...
@pytest.mark.parametrize(
    ...
    [
        ("19.1.3", {"gui_version": ""}),
        ("19.1.3", {"gui_version": "1.2.3"}),
        ("19.1.3", {"gui_version": "19.1.3"}),
        ("19.1.3", {"gui_version": "19.1.3.dev292+gc75d85b3c-dirty"}),
        ("19.1.3.post0", {"gui_version": "19.1.3"}),
        ("19.1.3.post0", {"gui_version": "19.1.3.dev292+gc75d85b3c-dirty"}),
        ("19.1.3.dev292", {"gui_version": "19.1.3"}),
        ("19.1.3.dev292", {"gui_version": "19.1.3.post0"}),
        ("19.1.3", {"gui_version": "invalid version string-..!@#$%^&*()"}),
        ("unknown", {}),
    ],
)
def test_mfix_version_no_warning(version, comments, mfix_gui):
    mfixgui.gui.__version__ = version
    mfix_gui.project.mfix_gui_comments = comments
    mfix_gui.check_version()
    mfix_gui.message.assert_not_called()
