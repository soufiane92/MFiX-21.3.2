...
cgs_to_SI = {
  ...
  'bc_t_s':         1,    # K                      : Solids phase temperature at the BC plane.
  'bc_theta_m':     0.0001,# cm^2/s^2 -> m^2/s^2   : Solids phase granular temperature at the BC plane.
  ...
  'fric_exp_pic':   1,    # max = 5.0              : Volume fraction exponential scale factor in frictional
  'fric_non_sing_fac':1,  # max = 0.0001           : Non-singularity term in frictional stress model.
  ...
  'ic_ep_s':        1,    # volume fraction        : Initial solids volume fraction of solids phase in the IC
  'ic_gama_rg':     None, # UNKNOWN                : Gas phase radiation coefficient in the IC region. Modify
  'ic_gama_rs':     None, # UNKNOWN                : Solids phase radiation coefficient in the IC region. Modify
  ...
  'ic_rop_s':       1000.0,# g/cm^3 -> kg/m^3      : Initial bulk density (rop_s = ro_s x ep_s) of solids phase
  ...
  'ic_t_rs':        1,    # K                      : Solids phase radiation temperature in the IC region.
  'ic_t_s':         1,    # K                      : Initial solids phase temperature in the IC region.
  'ic_theta_m':     0.0001,# cm^2/s^2 -> m^2/s^2   : Initial solids phase granular temperature in the IC
  ...
  'monitor_dt':     1,    # s                      : Interval to collect monitor data.
  'monitor_x_e':    0.01, # cm -> m                : X coordinate of the monitor region east face.
  'monitor_x_w':    0.01, # cm -> m                : X coordinate of the monitor region west face.
  'monitor_y_n':    0.01, # cm -> m                : Y coordinate of the monitor region north face.
  'monitor_y_s':    0.01, # cm -> m                : Y coordinate of the monitor region south face.
  'monitor_z_b':    0.01, # cm -> m                : Z coordinate of the monitor region bottom face.
  'monitor_z_t':    0.01, # cm -> m                : Z coordinate of the monitor region top face.
  ...
  'mppic_velfac_coeff':1, # max = 1.0              : Solids slip velocity scale factor. This term can be used to
  ...
  'psfac_fric_pic': 1,    # max = 10000.0          : Pressure linear scale factor in frictional stress model.
  ...
  'report_solid_inventory_dt':1,# s                : Interval at which solid inventory is reported (seconds).
  ...
  'tol_merge':      1,    # tolerance              : Tolerance used to remove duplicate nodes (expressed as a
...
def main():
    ...
    # We're not going to worry about any non-float keywords
    dp_keys = sorted([key for key, entry in doc.items() if entry['dtype'] == 'DP'])
    ...
    infile = open(__file__, 'r', encoding='utf-8', errors='replace')
    ...
    outfile = open(__file__ + '.tmp', 'w', encoding='utf-8', errors='replace')
