...
class Interpreter(object):
    ...
    def init_interpreter(self):
        ...
        le.returnPressed.connect(self.handle_interp_line)
        self.PS1 = '>>> '
        self.PS2 = '... '
    ...
    def setup_interpreter(self):
        ...
        try:
            ...
            installed = False
        module_name = "mfixgui.gui" if installed else "__main__"
        prelude = ["from %s import *" % module_name,
                   "g = self = gui",
                   "p = g.project",
                   "import mfixgui.namespace",
                   "mfixgui.namespace.gui = gui",
                   "from pprint import pformat",
                   "def doc(k): print(pformat(g.keyword_doc.get(k)))",
                   ""
        ]

        user_init = os.path.expanduser('~/.mfix.py')
        if os.path.exists(user_init):
            prelude += [line for line in open(user_init)]
            prelude.append("")
        for line in prelude:
