...
!      RVS = 1.26d9 * exp(-2.52d3/xTg) * EP_g(IJK) * c_SiH2 * c_H2
      FWD = A1F * exp(-2.56d4/xTg) * EP_g(IJK) * c_SiH4
      RVS = A1R * exp(-2.52d3/xTg) * EP_g(IJK) * c_SiH2 * c_H2
...
!      RVS = 1.00d11 * exp(-2.27d3/xTg) * EP_g(IJK) * c_SiH4 * c_SiH2
      FWD = A2F * exp(-2.63d4/xTg) * EP_g(IJK) * c_Si2H6
      RVS = A2R * exp(-2.27d3/xTg) * EP_g(IJK) * c_SiH4 * c_SiH2
...
! Total surface area of solids in computational cell (cm^2/cm^3)
         Sa = 6.0d0 * EP_s(IJK,1) / D_p0(1)   ! (1/m)
