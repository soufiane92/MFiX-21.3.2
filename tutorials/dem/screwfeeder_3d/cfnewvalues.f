...
!-----------------------------------------------
      ...
      DOUBLE PRECISION :: OMEGA_RB, Y0,Z0


! Rigid body rotation: angular velocity
! screw_omega contains the time-dependent omega interpolated using keyframe data
      OMEGA_RB = screw_omega(1)    ! OMEGA_RB vector points in the +x direction
      Y0 = 0.0875D0
      Z0 = 0.0D0

! The translational velocity will be overwritten for the screw particles:
! u = 0.0
! v = -OMEGA_RB*r*cos(theta) = -OMEGA_RB*DZ, where DZ is the z-distance from the axis of rotation
! w =  OMEGA_RB*r*sin(theta) =  OMEGA_RB*DY, where DZ is the y-distance from the axis of rotation

! The angular velocity will be overwritten for the screw particles:
! omega_x = omega_RB
! omega_y = 0.0
! omega_z = 0.0
...
! Overwrite u for screw particles
         WHERE(DES_USR_VAR(1,:) == 1.0D0) DES_VEL_NEW(:,1) = 0.0D0
...
! Overwrite v for screw particles         
         WHERE(DES_USR_VAR(1,:) == 1.0D0) DES_VEL_NEW(:,2) =           &
             -OMEGA_RB*(DES_POS_NEW(:,3) - Z0)
...
! Overwrite w for screw particles
         WHERE(DES_USR_VAR(1,:) == 1.0D0) DES_VEL_NEW(:,3) =           &
              OMEGA_RB*(DES_POS_NEW(:,2) - Y0)
...
! Overwrite omega_x for screw particles
         WHERE(DES_USR_VAR(1,:) == 1.0D0) OMEGA_NEW(:,1) =  OMEGA_RB
...
! Overwrite omega_y for screw particles
         WHERE(DES_USR_VAR(1,:) == 1.0D0) OMEGA_NEW(:,2) =  0.0D0  
...
! Overwrite omega_z for screw particles
         WHERE(DES_USR_VAR(1,:) == 1.0D0) OMEGA_NEW(:,3) =  0.0D0  
