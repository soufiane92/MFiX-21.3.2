...
!$omp    norm_previous_facet_coh, cohmag_previous_facet,bc_id,         &
...
!$omp    pijk,dg_pijk,des_pos_new,dynamic_bc,   &
...
!$omp    des_etan_wall,des_etat_wall,dtsolid,fc,norm_face,wall_tang_vel,  &
...
! Get BC_ID
            ...
            if(bc_id>0) then
               if(dynamic_bc(bc_id)) then
! Get specific tangential wall for each dynamic _bc
                  call compute_tang_vel(closest_pt(:), wall_tang_vel, bc_id)
               else
                  wall_tang_vel(:) = zero
               endif
