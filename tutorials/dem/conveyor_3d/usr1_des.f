...
! time)
! Here, belt velocities are computed for different BCs
            belt_vel(i,:) = kf_data(i)%var_current(1)*belt_dir(i,:)
          end do
        end if

! Broadcast the computed belt velocity to other processes
        call bcast(belt_vel, pe_io)
        return
