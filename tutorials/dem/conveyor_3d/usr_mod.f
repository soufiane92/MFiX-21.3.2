...
!     user defined subroutines (usr0, usr1, and usr2) set keyword CALL_USR to true.
    ...
    IMPLICIT NONE
! Array of velocities of various conveyor belts
      double precision, allocatable, dimension(:,:) :: belt_vel
! Array of directions of various conveyor belts
      double precision, allocatable, dimension(:,:) :: belt_dir

! Read user defined variable table from file or not (logical)
      logical :: keyframe_from_file
...
!                                                                      !
!  Purpose: Return the corresponding belt velocity                     !
!                                                                      !
!----------------------------------------------------------------------!
...
! Access the (contiguous) index corresponding to BC ID, since BC IDs
! can be non-contiguous
        ind = list(bc_id)

! Return corresponding belt velocity
! Here it does not depend on the location, it is only a function of time
! The belt velocity is computed in usr1_des.f from the keyframe data
        wall_tang_vel = belt_vel(ind,:)
