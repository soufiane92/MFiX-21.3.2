...
! local variables within source functions
      double precision      :: ros, mug, rog, MW, Rg, mus, Cpg, kg, &
                                Cps, ks
      double precision      :: Tg, Ts, epg, eps
      double precision      :: DTgDx, DTgDy, DTgDz, DTsDx, DTsDy, DTsDz, &
                                DepgDx, DepgDy, DepgDz, &
                                DepsDx, DepsDy, DepsDz
...
!     Inclusion of void fraction in the diffusion term inside the divergence operator
!     Avinash Vaidheeswaran
!     October 2017
! ------------------------------

      Tg = Tg0 + Tgx*Cos(aTgx*Pi*xt) + Tgxy*Cos(aTgxy*Pi*xt*yt) + Tgy*Cos(aTgy*Pi*yt) &
           + Tgyz*Sin(aTgyz*Pi*yt*zt) + Tgz*Sin(aTgz*Pi*zt) + Tgzx*Cos(aTgzx*Pi*zt*xt)

      Ts = Ts0 + Tsx*Cos(aTsx*Pi*xt) + Tsxy*Cos(aTsxy*Pi*xt*yt) + Tsy*Cos(aTsy*Pi*yt) &
           + Tsyz*Sin(aTsyz*Pi*yt*zt) + Tsz*Sin(aTsz*Pi*zt) + Tszx*Cos(aTszx*Pi*zt*xt)

      epg = 1.0d0 - (es0 + esx*cos(aesx*pi*xt) + esy*cos(aesy*pi*yt) + &
         esz*sin(aesz*pi*zt))

      eps = es0 + esx*cos(aesx*pi*xt) + esy*cos(aesy*pi*yt) + &
         esz*sin(aesz*pi*zt)

      DTgDx = - aTgx*Pi*Tgx*Sin(aTgx*Pi*xt) - aTgxy*yt*Pi*Tgxy*Sin(aTgxy*Pi*xt*yt) &
              - aTgzx*zt*Pi*Tgzx*Sin(aTgzx*Pi*zt*xt)
      DTgDy = - aTgy*Pi*Tgy*Sin(aTgy*Pi*yt) - aTgxy*xt*Pi*Tgxy*Sin(aTgxy*Pi*xt*yt) &
              + aTgyz*zt*Pi*Tgyz*Cos(aTgyz*Pi*yt*zt)
      DTgDz = aTgz*Pi*Tgz*Cos(aTgz*Pi*zt) - aTgzx*xt*Pi*Tgzx*Sin(aTgzx*Pi*zt*xt) &
              + aTgyz*yt*Pi*Tgyz*Cos(aTgyz*Pi*yt*zt)

      DTsDx = - aTsx*Pi*Tsx*Sin(aTsx*Pi*xt) - aTsxy*yt*Pi*Tsxy*Sin(aTsxy*Pi*xt*yt) &
              - aTszx*zt*Pi*Tszx*Sin(aTszx*Pi*zt*xt)
      DTsDy = - aTsy*Pi*Tsy*Sin(aTsy*Pi*yt) - aTsxy*xt*Pi*Tsxy*Sin(aTsxy*Pi*xt*yt) &
              + aTsyz*zt*Pi*Tsyz*Cos(aTsyz*Pi*yt*zt)
      DTsDz = aTsz*Pi*Tsz*Cos(aTsz*Pi*zt) - aTszx*xt*Pi*Tszx*Sin(aTszx*Pi*zt*xt) &
              + aTsyz*yt*Pi*Tsyz*Cos(aTsyz*Pi*yt*zt)

      DepgDx = aesx*esx*Pi*Sin(aesx*Pi*xt)
      DepgDy = aesy*esy*Pi*Sin(aesy*Pi*yt)
      DepgDz = -aesz*esz*Pi*Cos(aesz*Pi*zt)

      DepsDx = -aesx*esx*Pi*Sin(aesx*Pi*xt)
      DepsDy = -aesy*esy*Pi*Sin(aesy*Pi*yt)
      DepsDz = aesz*esz*Pi*Cos(aesz*Pi*zt)
