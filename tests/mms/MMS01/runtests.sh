...
echo "******** Running mesh_8..."
"${MFIXSOLVER}" -f mfix.dat imax=8 jmax=8 #> out.log
...
"${MFIXSOLVER}" -f mfix.dat imax=16 jmax=16 >out.log
cat "$CASE_DIR"/de_norms.dat >>"$CASE_DIR"/de_norms_collected.dat
rm -f "$CASE_DIR"/{MMS2D.*,de_norms.dat,out.log}

# Run mesh_32 (i.e., 32x32 for 2D, 32x32x32 for 3D)
echo "******** Running mesh_32..."
mpirun --oversubscribe -np 4 "${MFIXSOLVER}" -f mfix.dat imax=32 jmax=32 nodesi=2 nodesj=2 nodesk=1 >out.log
cat "$CASE_DIR"/de_norms.dat >>"$CASE_DIR"/de_norms_collected.dat
rm -f "$CASE_DIR"/{MMS2D.*,de_norms.dat,out.log}
#mkdir mesh_32
...
#mpirun -np 16 ${MFIXSOLVER} -f mfix.dat imax=64 jmax=64 nodesi=4 nodesj=4 nodesk=1 > out.log
#cat "$CASE_DIR"/de_norms.dat >> "$CASE_DIR"/de_norms_collected.dat
#rm -f "$CASE_DIR"/{MMS2D.*,de_norms.dat,out.log}
##mkdir mesh_64
...
#mpirun -np 32 ${MFIXSOLVER} -f mfix.dat imax=128 jmax=128 nodesi=8 nodesj=4 nodesk=1 > out.log
#cat "$CASE_DIR"/de_norms.dat >> "$CASE_DIR"/de_norms_collected.dat
#rm -f "$CASE_DIR"/{MMS2D.*,de_norms.dat,out.log}
