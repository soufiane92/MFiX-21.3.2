...
mpirun --oversubscribe -np 1 "${MFIXSOLVER}" -f mfix.dat imax=8 jmax=8 kmax=8 DISCRETIZE=9*2 UR_FAC=9*0.05 >out.log
...
mpirun --oversubscribe -np 1 "${MFIXSOLVER}" -f mfix.dat imax=16 jmax=16 kmax=16 >out.log
cat "$CASE_DIR"/de_norms.dat >>"$CASE_DIR"/de_norms_collected.dat
rm -f "$CASE_DIR"/{MMS05.*,de_norms.dat,out.log}
rm -f "$CASE_DIR"/solution_*.dat
...
#mpirun -np 4 ${MFIXSOLVER} -f mfix.dat imax=32 jmax=32 kmax=32 nodesi=2 nodesj=2 nodesk=1 > out.log
#cat "$CASE_DIR"/de_norms.dat >> "$CASE_DIR"/de_norms_collected.dat
#rm -f "$CASE_DIR"/{MMS05.*,de_norms.dat,out.log}
#rm -f "$CASE_DIR"/solution_*.dat
...
#mpirun -np 64 ${MFIXSOLVER} -f mfix.dat imax=64 jmax=64 kmax=64 nodesi=4 nodesj=4 nodesk=4 > out.log
#cat "$CASE_DIR"/de_norms.dat >> "$CASE_DIR"/de_norms_collected.dat
#rm -f "$CASE_DIR"/{MMS05.*,de_norms.dat,out.log}
#rm -f "$CASE_DIR"/solution_*.dat
...
#mpirun -np 64 ${MFIXSOLVER} -f mfix.dat imax=128 jmax=128 kmax=128 nodesi=4 nodesj=4 nodesk=4 > out.log
#cat "$CASE_DIR"/de_norms.dat >> "$CASE_DIR"/de_norms_collected.dat
#rm -f "$CASE_DIR"/{MMS05.*,de_norms.dat,out.log}
#rm -f "$CASE_DIR"/solution_*.dat
