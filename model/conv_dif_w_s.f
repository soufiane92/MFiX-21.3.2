...
MODULE CONV_DIF_W_S_Mod
   ...
   USE dif_w_is_mod, only: dif_w_is
   USE fldvar, only: u_s, v_s, w_s
   USE fun_avg, only: avg_z, avg_z_t, avg_z_h, avg_x_h, avg_y_h
   USE functions, only: bottom_of, east_of, north_of, top_of, west_of
   USE functions, only: flow_at_t, wall_at
   USE functions, only: im_of, jm_of, km_of
   USE functions, only: ip_of, jp_of, kp_of
   USE functions, only: north_of, south_of
   USE geometry, only: ayz_w, axz_w, axy_w, ayz_w, axz_w
   USE geometry, only: ody_n, odx_e, odz, do_k, ox, ox_e
   ...
   USE run, only: def_cor, discretize, ghd_2007, kt_type_enum, momentum_z_eq
