MODULE SOURCE_W_G_MOD

   USE bc
   USE bodyforce, only: bfz_g
   USE cg_source_w_g_mod, only: cg_source_w_g, cg_source_w_g_bc
   USE compar, only: ijkstart3, ijkend3, imap
   USE cutcell, only: a_wpg_b, a_wpg_t
   USE cutcell, only: blocked_w_cell_at
   USE cutcell, only: cartesian_grid, cut_w_treatment_at
   USE drag, only: f_gs, beta_ij
   USE fldvar, only: ep_g, ep_s, epg_jfac, k_turb_g
   USE fldvar, only: p_g, ro_g, rop_g, rop_go, rop_s
   USE fldvar, only: v_g, w_g, w_go, u_s, v_s, w_s, w_so, u_g
   USE fun_avg, only: avg_x, avg_z, avg_y
   USE fun_avg, only: avg_x_h, avg_x_e, avg_y_n, avg_z_t, avg_z_h
   USE functions
   ...
   USE ps
   USE run, only: ghd_2007, hys, shear, v_sh
   ...
   USE wall_function_mod, only: wall_function
   USE tau_g, only: tau_w_g
   USE toleranc, only: dil_ep_s
   USE turb, only: k_epsilon
   USE turb, only: turb_c_mu, turb_kappa
   USE visc_g, only: epmu_gt, mu_gt
   USE vshear, only: vsh
