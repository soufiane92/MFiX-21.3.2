MODULE CALC_TAU_V_G_MOD

   USE bc
   USE cutcell
   USE fldvar, only: ep_g, u_g, v_g, w_g
   USE fun_avg, only: avg_y, avg_x_h, avg_y_h, avg_z_h
   ...
   USE visc_g, only: df_gv, trd_g, epmu_gt, lambda_gt, eplambda_gt
...
!$omp         ep_g, lambda_gt, trd_g, epmu_gt, u_g, v_g, w_g,          &
!$omp         ayz_v, axz_v, axz, axy_v, dil_ep_s,                      &
!$omp         ody_n, ody, eplambda_gt)
...
! delta (lambda.trcD)Ap|N-S  : at (i, j+1 - j-1, k)
               SBV = (EPLAMBDA_GT(IJKN)*TRD_G(IJKN)-&
                      EPLAMBDA_GT(IJK)*TRD_G(IJK))*AXZ(IJK)
...
!$omp         oneody_n_v, oneody_n_w, oneody_n_u, dil_ep_s,            &
!$omp         cut_v_cell_at, wall_u_at, wall_w_at, area_v_cut,         &
!$omp         blocked_u_cell_at, blocked_w_cell_at, eplambda_gt)
...
! bulk viscosity term
            SBV =  (EPLAMBDA_GT(IJKN)*TRD_G(IJKN)) * AXZ_V(IJK) - &
                   (EPLAMBDA_GT(IJK) *TRD_G(IJK) ) * AXZ_V(IJMK)
