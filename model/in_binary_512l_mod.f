...
!                                                                      C
!  Module name: IN_BIN_512L                                            C
!  Purpose: read an array in chunks of 512 bytes (LOGICAL WORDS)       C
...
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^C
!
MODULE IN_BINARY_512L

CONTAINS

      SUBROUTINE IN_BIN_512L(IUNIT, ARRAY, NN, NEXT_REC)
...
!
      ...
      RETURN
      END SUBROUTINE IN_BIN_512L

      subroutine convert_from_io_l(arr_io,arr_internal,nn)
      ...
      logical, intent(in) :: arr_io(:)
      logical, intent(out) :: arr_internal(:)
...
!           write(*,*)i,j,k,ijk, ijk_io
      ...
      return
    end subroutine convert_from_io_l

      subroutine convert_to_io_l(arr_internal,arr_io,nn)
      ...
      return
    end subroutine convert_to_io_l
