...
MODULE SOLVE_LIN_EQ_MOD

   use compar
   use dgtsv_mod, only: dgtsv
   use error_manager
   use geometry
   use ghdmassflux_mod, only: ghdmassflux
   use leq_bicgs_mod, only: leq_bicgs
   use leq_cg_mod, only: leq_cg
   use leq_gmres_mod, only: leq_gmres
   use leq_sor_mod, only: leq_sor
   use leqsol
   use param
   use param1
   use residual
...
! BICGSTAB: BIConjugate Gradients STabilized method
         IF(do_transpose) THEN  ! keyword default=false
