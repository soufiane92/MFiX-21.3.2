MODULE BC_PHI_MOD
   ...
   USE bc, only: bc_defined, bc_plane, bc_type_enum, outflow
   USE bc, only: bc_i_e, bc_i_w
   USE bc, only: bc_j_n, bc_j_s
   USE bc, only: bc_k_b, bc_k_t
   USE bc, only: bc_type_enum, none, mass_inflow, mass_outflow, no_slip_wall
   USE bc, only: cg_nsw, cg_fsw, cg_psw, cg_mi, cg_po
   USE bc, only: p_inflow, p_outflow, par_slip_wall, free_slip_wall
   USE compar, only: dead_cell_at, ijkstart3, ijkend3
   USE cutcell, only: blocked_cell_at, cut_cell_at
   USE cutcell, only: cartesian_grid, cg_safe_mode, bc_id
   USE functions, only: default_wall_at, flow_at, fluid_at
   USE functions, only: funijk, is_on_mype_plus2layers
   ...
   USE geometry, only: odx_e, ody_n, odz_t, ox, do_k
   USE indices, only: im1, jm1, km1, i_of, j_of, k_of
   USE param, only: bottom, top, east, west, north, south
   USE param, only: dimension_3, dimension_bc, dimension_m
