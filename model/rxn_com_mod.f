...
! Length of noncomment string
      ...
      CHARACTER(len=128) :: species_inc_path

#ifdef SPECIES_INC
      species_inc_path = SPECIES_INC
#else
! if no path to species.inc, solver was not built for any chemical reactions
      RETURN
#endif
...
! No species.inc file was located.
         CASE DEFAULT
            IF(.NOT.REINITIALIZING)THEN
               WRITE(ERR_MSG, 1004)species_inc_path
               CALL LOG_WARNING()
            ENDIF
            EXIT SRC_LP
         END SELECT

 1004 FORMAT('Warning 1004: Unable to locate ', A, /,&
         'Cannot verify species aliases or reaction names.')
...
! We only want to process lines that have = as the other are comments.
            POS = INDEX(INPUT,"=")
...
! Reaction Names for discrete solids
 ...
 1300 FORMAT('Error 1300: An entry in the species.inc file does not ', &
         'match any inputs',/'in input deck.'/3x,'Name: ',A,4x, &
...
! Finalize the error message.
      IF(CHECK_DATABASE) CALL LOG_MESSAGE(__FILE__, __LINE__, LOGLEVEL_INFO, HEADER=.FALSE.)
...
! Calculate mass of reactants and products. Used to ensure mass balance.
         IF(MWxStoich < ZERO) THEN
            rSUM = rSUM - MWxStoich
            IF(M /= 0 .and. RxN%Calc_DH) THEN
               sprCount = sprCount + 1
               IF(sprCount == 1) THEN
                  sprIndex = M
! Verify that there is at most one solids phase file (reactant).
               ELSEIF( M /= sprIndex) THEN
                  IF(DMP_LOG) THEN
                     WRITE(ERR_MSG,1002) trim(CALLER), trim(RxN%Name)
                     IER = 1
                  ENDIF
               ENDIF
            ENDIF
...
! Verify that the mass of products equals reactants: (Mass Balance)
      IF (.NOT.COMPARE(rSUM,pSUM)) THEN
         IF(DMP_LOG) THEN
            WRITE(ERR_MSG,1001) trim(CALLER), trim(RxN%Name), rSUM, pSUM
            IER = 1
         ENDIF
      ENDIF
...
! other phase's species equations are solved.
                  ...
                  IF(abs(RxN%rPhase(LM)) > SMALL_NUMBER) THEN
                     IF((lSEq(toPhase) .AND. .NOT.lSEq(M)) .OR. &
                        ...
                        IF(DMP_LOG) THEN
                           WRITE(ERR_MSG,1001) trim(CALLER)
                           IF(lSEq(M)) THEN
                              WRITE(ERR_MSG,1101) M, 'Solving'
                              WRITE(ERR_MSG,1101) toPhase, &
                                 'Not Solving'
                           ELSE
                              WRITE(ERR_MSG,1101) M, 'Solving'
                              WRITE(ERR_MSG,1101) toPhase, &
...
! The gas phase is the destination phase.
                     IF(toPhase == 0) THEN
! Counter for the number of solids phases transferring mass to the
! gas phase.
                        mCount = 0
...
! The fraction of material transferred from phase 0 to phase MM.
...
! Increment the number of phases the gas receives mass from.
                        ...
                        IF(mCount /= 1) THEN
                           IF(DMP_LOG) THEN
                              WRITE(ERR_MSG,1002) trim(CALLER), &
                                 trim(RxN%ChemEq)
                              WRITE(ERR_MSG,1000)
                           ENDIF
                           CALL WRITE_RXN_SUMMARY(RxN, lSAg(:), &
...
! Indicate that phase MM receives mass from phase M.
                              RxN%Species(lN)%mXfr = MM
! Calculate the fraction of material consumed from phase M is transferred
! to phase MM.
                              RxN%Species(lN)%xXfr =  &
                                 abs(lnMT(MM) / lnMT(LL))
                  ...
                  IF(abs(RxN%rPhase(LM)) > SMALL_NUMBER) THEN
                     IF((lSEq(fromPhase) .AND. .NOT.lSEq(M)) .OR.   &
                        ...
                        IF(DMP_LOG) THEN
                           ...
                           IF(lSEq(M)) THEN
                              WRITE(ERR_MSG,1101) M, 'Solving'
                              WRITE(ERR_MSG,1101) fromPhase, &
                                 'Not Solving'
                           ELSE
                              WRITE(ERR_MSG,1101) fromPhase, 'Solving'
                              WRITE(ERR_MSG,1101) M, 'Not Solving'
                           ENDIF
                           WRITE(ERR_MSG,1000)
                        ENDIF
                        CALL WRITE_RXN_SUMMARY(RxN, lSAg(:),           &
...
! Mass fraction of transferred material.
! This variable is not currently used for gas/solids reactions.
                        RxN%Species(lN)%xXfr = ZERO
                     ENDIF
! Solids/Solids mass transfer: Enthalpy transfer from mass transfer is
...
! Verify that a catalyst phase was found.
               IF(catPhase == -1) THEN
                  IF(DMP_LOG) THEN
                     WRITE(ERR_MSG,1003) trim(CALLER), &
...
! Verify that a reacting phase was found.
               IF(toPhase == -1) THEN
                  IF(DMP_LOG) THEN
                     WRITE(ERR_MSG,1003) trim(CALLER), 'reacting', &
                        trim(RxN%Name)
                     WRITE(ERR_MSG,1000)
                  ENDIF
...
! Something when wrong.
               IF(catPhase == toPhase) THEN
                  IF(DMP_LOG) THEN
                     WRITE(ERR_MSG,1004) trim(CALLER), trim(RxN%Name)
                     WRITE(ERR_MSG,1000)
                  ENDIF
                  CALL WRITE_RXN_SUMMARY(RxN, lSAg(:), lSAs(:,:),.TRUE.)
...
! concluded.
            ...
            WRITE(ERR_MSG,1002) trim(CALLER), trim(RxN%ChemEq)
            WRITE(ERR_MSG,1000)
            call log_error()
         ENDIF
      ENDIF
