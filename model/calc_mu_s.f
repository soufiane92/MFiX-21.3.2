...
MODULE CALC_MU_s_MOD
   ...
   USE param1, only: zero, one, large_number, small_number, half, undefined
   ...
   USE run, only: friction, schaeffer, princeton, guo_boyce
   ...
   USE trace, only: trD_s2_ip, trd_s2, trd_s_c
   USE transport_coeff_ghd_mod, only: transport_coeff_ghd
   ...
   USE visc_s, only: i2_devD_s
   USE visc_s, only: lambda_s, lambda_s_c, mu_b_v
   USE visc_s, only: mu_s, epmu_s, lambda_s, eplambda_s, mu_s_c
...
! Princeton's frictional implementation
      IF (FRICTION .AND. PRINCETON .AND. CLOSE_PACKED(M)) call friction_princeton(M)
! Guo-Boyce's frictional implementation
      IF (FRICTION .AND. GUO_BOYCE .AND. CLOSE_PACKED(M)) call friction_guo_boyce(M)
...
! and blended in its own routine to accommodate slightly different
...
! Bulk and shear viscosity
            Mu_s_v(IJK) = (5.d0/96.d0)*D_p(IJK,M)*RO_S(IJK,M)* &
               DSQRT(Pi*Theta_m(IJK,M))
...
!               Mu_star = EP_s(IJK,M)*G_0(IJK,M,M)*Mu_s_v(IJK)/ &
!                  SUM_EpsGo
...
! Second viscosity as defined in MFIX
            ...
            IF(SWITCH == ZERO .OR. RO_G(IJK) == ZERO) THEN
               Kth_star=Kth
! this seems to be the proper extension if ro_g0=0 for M>1
!               Kth_star = EP_s(IJK,M)*G_0(IJK,M,M)*Kth/ &
!                  SUM_EpsGo
            ELSEIF(Theta_m(IJK,M) .LT. SMALL_NUMBER)THEN
               Kth_star = ZERO
            ELSEIF(EP_S(IJK,M) < DIL_EP_S) THEN
...
!
      ...
      DOUBLE PRECISION :: dZeta_dT, TdGama_dT, NuK, Kth0, KthK
...
!            dZeta_dT = -0.5d0*xsi_gtsh(ijk)/(M_pm*theta_m(ijk,m))
             ...
             TdGama_dT = 3d0*pi*D_PM**2*RO_g(ijk)*K_phi(EP_SM)* &
                dsqrt(theta_m(ijk,m))/(2d0*M_pm)
...
! Finally notice that conductivity K in eq (7.10) must be
...
! corrections due to W. Fullmer
             Kphidphi = (0.5d0 + 3.6d0*EP_SM/(one-EP_SM))*K_phi(EP_SM)
...
! Finally compute the Dufour coefficient Mu (Kphi_s(IJK,M)) from eq (7.22) GTSH
...
!  calculation of granular stress terms and granular conductivity      C
!                                                                      C
!  Author: Janine E. Galvin, University of Colorado                    C
!                                                                      C
!  Literature/Document References:                                     C
...
!               Mu_star = Mu_s_dil*G_0(IJK,M,M)*EP_s(IJK,M)/&
!                  SUM_EpsGo
...
! Find the granular conductivity
            ...
            IF(SWITCH == ZERO .OR. RO_G(IJK) == ZERO) THEN
               Kth_star = K_s_dil
! this seems to be the proper extension for M>1 when ro_g=0
!               Kth_star = K_s_dil*G_0(IJK,M,M)*EP_s(IJK,M)/&
!                  SUM_EpsGo
...
! added together so do not explicitly include them
! in calculations
...
! again these terms cancel so do not explicitly include
...
! plastic pressure (p_star) is calculated separately to ensure
...
! used to compute frictional terms
      DOUBLE PRECISION :: mu_sf, Pc, PfoPc, N_Pff, PfoPc_max
...
! sum_eps_cp calculation for polydispersity and dp_avg calc. for ghd_07
               SUM_EPS_CP = ZERO
               dp_avg = ZERO
               DO MM=1,SMAX
...
! Lun et al. (1984). So quantities are reproduced here based on this KT.
...
! the selected KT.
                  ...
                  DO MM = 1, SMAX
                     SUM_EpsGo =  SUM_EpsGo+EP_s(IJK,MM)*G_0(IJK,M,MM)
                  ENDDO
                  Mu_sv = (5.d0*DSQRT(Pi*Theta_m(IJK,M))*D_p(IJK,M)*&
                     RO_S(IJK,M))/96.d0
                  Mu_bv = (256.d0*Mu_sv*EP_s(IJK,M)*SUM_EpsGo)&
                     /(5.d0*Pi)
                  IF(SWITCH == ZERO .OR. RO_G(IJK) == ZERO) THEN
                     Mu_star = Mu_sv
! this seems to be the proper extension if ro_g0=0 for M>1
                     Mu_star = EP_s(IJK,M)*G_0(IJK,M,M)*Mu_sv/ &
                        SUM_EpsGo
                  ELSEIF(Theta_m(IJK,M) .LT. SMALL_NUMBER)THEN
                     Mu_star = ZERO
                  ELSE
...
! simply to avoid calculation in else branch below
               ...
               mu_sf = DSQRT(2d0)*P_s_f(IJK)*Sin_Phi*(N_Pff - (N_Pff-1d0)*&
                  (PfoPc)**(1d0/(N_Pff-1d0)))
...
!vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvC
!                                                                      C
!  Subroutine: FRICTION_GUO-BOYCE                                      C
!  Purpose: Calculate frictional contributions to granular stress      C
!  terms based on model by Guo_Boyce                                   C
!  This is modified form of Srivastava & Sundaresan (2003)             C
!                                                                      C
!  Author: Qiang Guo, Christopher Boyce             Date: 08-AUG-21    C
!                                                                      C
!  Literature/Document References:                                     C
!  Qiang Guo, Yuxuan Zhang, Azin Padash, Kenan Xi, Thomas M. Kovar,    C
!  Christopher M.  Boyce, "Dynamically structured bubbling in vibrated C
!  gas-fluidized granular materials", Proceedings of the National      C
!  Academy of Sciences Aug 2021, 118 (35) e2108647118;                 C
!  DOI: 10.1073/pnas.2108647118                                        C
!                                                                      C
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^C
      Subroutine Friction_guo_boyce(M)
...
! This is NOT done when mixture mom. eq. is solved (i.e. for GHD theory)
      ...
      ENDDO   ! IJK loop
      RETURN
      END SUBROUTINE FRICTION_GUO_BOYCE
