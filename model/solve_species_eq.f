MODULE SOLVE_SPECIES_EQ_MOD
   ...
   use check, only: check_conservation
   use chischeme, only: set_chi, unset_chi
   use compar, only: ijkstart3, ijkend3, mype, numpes
   use conv_dif_phi_mod, only: conv_dif_phi
   use fldvar, only: ep_g, u_g, v_g, w_g, x_g, x_go, rop_g, rop_go
   use fldvar, only: ep_s, u_s, v_s, w_s, x_s, x_so, rop_s, rop_so
   ...
   use source_phi_mod, only: source_phi, point_source_phi
...
! local array to store product of diffusion coefficient and
! _bulk_ density (densityxvolume fraction)
      DOUBLE PRECISION :: DFxROP(DIMENSION_3)
...
! calculate the source terms to be used in the a matrix and b vector
                  ...
                  S_C(IJK) = APO*X_GO(IJK,LN) + &
                     ...
                     R_GP(IJK,LN)*VOL(IJK)
! multiple mass diffusivity (dimension length^2/time) by density for
! mass flux and by volume fraction for implementation in multiphase
! framework.
                  DFxROP(IJK)=DIF_G(IJK,LN)*ROP_G(IJK)
               ELSE
                  S_P(IJK) = ZERO
                  S_C(IJK) = ZERO
                  DFxROP(IJK) = ZERO  ! same as in calc_dif_g
               ENDIF
            ENDDO

! calculate the convection-diffusion terms
            IF(.NOT.ADDED_MASS) THEN
               CALL CONV_DIF_PHI (X_G(1,LN), DFxROP, &
...
!!$omp    parallel do private(IJK, APO)
               DO IJK = ijkstart3, ijkend3
                  IF (FLUID_AT(IJK)) THEN
                     ...
                     EPS(IJK) = EP_S(IJK,M)
! see note above for gas phase
                     DFxROP(IJK) = DIF_S(IJK,M,LN)*ROP_S(IJK,M)
                  ELSE
                     S_P(IJK) = ZERO
                     S_C(IJK) = ZERO
                     EPS(IJK) = ZERO
                     DFxROP(IJK) = ZERO   ! same as in calc_dif_s
                  ENDIF
               ENDDO

               IF(.NOT.ADDED_MASS .OR. M /= M_AM) THEN
                  CALL CONV_DIF_PHI (X_S(1,M,LN), DFxROP, &
...
! unphysical values. To prevent them from propagating through the domain
...
END MODULE SOLVE_SPECIES_EQ_MOD
