MODULE WRITE_OUT0_MOD
   ...
   use machine
   use mpi_utility
   use out_array_c_mod, only: out_array_c
   use output
   use param, only: dimension_3
   use param1, only: undefined, half, zero, undefined_c
   use particle_filter
   use physprop
   use ps
   use run
...
! Header Line 2
 ...
 1450 FORMAT(/7X,'Use ',A,' collision model.',2/10X,&
...
! Point Sources
      WRITE (UNIT_OUT, 1750)
      DO L = 1, DIMENSION_PS
         IF (PS_DEFINED(L)) THEN
            WRITE (UNIT_OUT, 1751) L
            LOC(1) = LOCATION(PS_I_W(L),XMIN,DX) - HALF*DX(PS_I_W(L))
            LOC(2) = LOCATION(PS_I_E(L),XMIN,DX) + HALF*DX(PS_I_E(L))
            LOC(3) = LOCATION(PS_J_S(L),ZERO,DY) - HALF*DY(PS_J_S(L))
            LOC(4) = LOCATION(PS_J_N(L),ZERO,DY) + HALF*DY(PS_J_N(L))
            LOC(5) = LOCATION(PS_K_B(L),ZERO,DZ) - HALF*DZ(PS_K_B(L))
            LOC(6) = LOCATION(PS_K_T(L),ZERO,DZ) + HALF*DZ(PS_K_T(L))
            WRITE (UNIT_OUT, 1755) &
               PS_X_W(L), LOC(1), PS_X_E(L), LOC(2), &
               PS_Y_S(L), LOC(3), PS_Y_N(L), LOC(4), &
               PS_Z_B(L), LOC(5), PS_Z_T(L), LOC(6)
            WRITE (UNIT_OUT, 1756) &
               PS_I_W(L), PS_I_E(L), &
               PS_J_S(L), PS_J_N(L), &
               PS_K_B(L), PS_K_T(L)
            ...
            WRITE (UNIT_OUT, "(' ')")
            IF (PS_T_G(L) /= UNDEFINED) WRITE (UNIT_OUT, 1760) PS_T_G(L)
            IF (SPECIES_EQ(0) .AND. PS_X_G(L,1)/=UNDEFINED) THEN
               WRITE (UNIT_OUT, 1761)
               DO NN = 1, NMAX(0)
                  WRITE (UNIT_OUT, 1762) NN, PS_X_G(L,NN)
               END DO
            ENDIF

            WRITE (UNIT_OUT, "(' ')")
            WRITE (UNIT_OUT, 1763) PS_MASSFLOW_G(L)
            IF (PS_U_G(L) /= UNDEFINED) WRITE (UNIT_OUT, 1764) PS_U_G(L)
            IF (PS_V_G(L) /= UNDEFINED) WRITE (UNIT_OUT, 1765) PS_V_G(L)
            IF (PS_W_G(L) /= UNDEFINED) WRITE (UNIT_OUT, 1766) PS_W_G(L)

            DO M = 1, MMAX_TOT
               WRITE (UNIT_OUT, "(' ')")
               IF (PS_T_S(L,M) /= UNDEFINED) &
                  WRITE (UNIT_OUT, 1770) M, PS_T_S(L,M)
               IF (SPECIES_EQ(M) .AND. PS_X_S(L,M,1)/=UNDEFINED) THEN
                  WRITE (UNIT_OUT, "(' ')")
                  WRITE (UNIT_OUT, 1771) M
                  DO NN = 1, NMAX(M)
                     WRITE (UNIT_OUT, 1772) NN, PS_X_S(L,M,NN)
                  END DO
               ENDIF

               WRITE (UNIT_OUT, "(' ')")
               IF(PS_MASSFLOW_S(L,M) /= UNDEFINED) &
                  WRITE (UNIT_OUT, 1775) M, PS_MASSFLOW_S(L,M)
               IF(PS_U_S(L,M)/=UNDEFINED) &
                  WRITE(UNIT_OUT,1776)M, PS_U_S(L,M)
               IF(PS_V_S(L,M)/=UNDEFINED) &
                  WRITE(UNIT_OUT,1777)M, PS_V_S(L,M)
               IF(PS_W_S(L,M)/=UNDEFINED) &
                  WRITE(UNIT_OUT,1778)M, PS_W_S(L,M)
            END DO
 ...
 1750 FORMAT(//,3X,'X. POINT SOURCES')
 1751 FORMAT(/7X,'Point source no : ',I4)
 1755 FORMAT(9X,39X,' Specified  ',5X,' Simulated  ',/9X,&
         'X coordinate of west face   (PS_X_w) = ',G12.5,5X,G12.5/,9X,&
         'X coordinate of east face   (PS_X_e) = ',G12.5,5X,G12.5/,9X,&
         'Y coordinate of south face  (PS_Y_s) = ',G12.5,5X,G12.5/,9X,&
         'Y coordinate of north face  (PS_Y_n) = ',G12.5,5X,G12.5/,9X,&
         'Z coordinate of bottom face (PS_Z_b) = ',G12.5,5X,G12.5/,9X,&
         'Z coordinate of top face    (PS_Z_t) = ',G12.5,5X,G12.5)
 1756 FORMAT(9X,'I index of cell at west   (IS_I_w) = ',I4,/,9X,&
         'I index of cell at east   (PS_I_e) = ',I4,/,9X,&
         'J index of cell at south  (PS_J_s) = ',I4,/,9X,&
         'J index of cell at north  (PS_J_n) = ',I4,/,9X,&
         'K index of cell at bottom (PS_K_b) = ',I4,/,9X,&
         'K index of cell at top    (PS_K_t) = ',I4)

 1760 FORMAT(9X,'Gas temperature (PS_T_g) = ',G12.5)
 1761 FORMAT(9X,'Gas species',5X,'Mass fraction (PS_X_g)')
 1762 FORMAT(9X,3X,I3,15X,G12.5)

 1763 FORMAT(9X,'Gas mass flow rate (PS_MASSFLOW_g) = ',G12.5)
 1764 FORMAT(9X,'X-component of gas velocity (PS_U_g) = ',G12.5)
 1765 FORMAT(9X,'Y-component of gas velocity (PS_V_g) = ',G12.5)
 1766 FORMAT(9X,'Z-component of gas velocity (PS_W_g) = ',G12.5)

 1770 FORMAT(9X,'Solids phase-',I2,' temperature (PS_T_s) = ',G12.5)

 1771 FORMAT(9X,'Solids-',I2,' species',5X,'Mass fraction (PS_X_s)')
 1772 FORMAT(9X,3X,I3,20X,G12.5)
 1775 FORMAT(9X,'Solids phase-',I2,' mass flow rate (PS_MASSFLOW_s) =',G12.5)
 1776 FORMAT(9X,'X-component of solids phase-',I2,' velocity (PS_U_s) =',G12.5)
 1777 FORMAT(9X,'Y-component of solids phase-',I2,' velocity (PS_V_s) =',G12.5)
 1778 FORMAT(9X,'Z-component of solids phase-',I2,' velocity (PS_W_s) =',G12.5)
...
!
 ...
 1158 FORMAT(7X,'Underrelaxation (UR_FAC) and',&
