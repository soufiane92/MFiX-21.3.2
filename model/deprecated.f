...
! Old keyword for solids density :: Replaced by RO_s0
      ...
      LOGICAL :: MPPIC_GRAV_TREATMENT, MPPIC_PDRAG_IMPLICIT, PIC_REPORT_MIN_EPG

      INTEGER :: netCDF_file_index
      LOGICAL :: bWrite_netcdf(20),  bFirst_netcdf_write, bGlobalNetcdf
...
! 2016-2 Deprecated list:
!-----------------------------------------------------------------------
      NAMELIST / DEP_2016_2 / GRANULAR_ENERGY, FRICTION, SCHAEFFER,    &
         SAVAGE, BLENDING_STRESS, TANH_BLEND, SIGM_BLEND, AHMADI,      &
         SIMONIN

! 18.1 Deprecated list:
!-----------------------------------------------------------------------
      NAMELIST / DEP_18_1 / FPFOI, C_FAC,                              &
         BC_VELMAG_G, BC_VELMAG_S


! 18.2 Deprecated list:
!-----------------------------------------------------------------------
      NAMELIST / DEP_18_2 / IC_PIC_CONST_NPC, BC_PIC_MI_CONST_NPC,    &
           MPPIC_GRAV_TREATMENT, MPPIC_PDRAG_IMPLICIT,                &
           PIC_REPORT_MIN_EPG, MPPIC_COEFF_EN2

! 20.1 Deprecated list:
!-----------------------------------------------------------------------
      NAMELIST / DEP_20_1 / BWRITE_NETCDF

! 20.1 Release Deprecated keywords.
      STRING=''; STRING = '&DEP_20_1 '//trim(adjustl(INPUT))//'/'
      READ(STRING,NML=DEP_20_1,IOSTAT=IOS)
      IF(IOS == 0) CALL DEPRECATED(LINE_NO, INPUT, '20.1')

! 18.2 Release Deprecated keywords.
      STRING=''; STRING = '&DEP_18_2 '//trim(adjustl(INPUT))//'/'
      READ(STRING,NML=DEP_18_2,IOSTAT=IOS)
      IF(IOS == 0) CALL DEPRECATED(LINE_NO, INPUT, '18.2')

! 18.1 Release Deprecated keywords.
      STRING=''; STRING = '&DEP_18_1 '//trim(adjustl(INPUT))//'/'
      READ(STRING,NML=DEP_18_1,IOSTAT=IOS)
      IF(IOS == 0) CALL DEPRECATED(LINE_NO, INPUT, '18.1')
...
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^!
 ...
 1000 FORMAT(//1X,/' From DEPRECATED',/' Error 1000:',          &
         ' A keyword pair on line ',A,' of the project file was',/    &
...
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^!
      ...
      IF(myPE == 0) WRITE(err_msg,2000) trim(iVAL(LINE_NO)), trim(INPUT)

      call log_error()

 2000 FORMAT(//1X,/' From: UNKNOWN_KEYWORD',/' Error 2000: ',   &
         'Unable to process line ',A,' of the project file.',2/3x,    &
