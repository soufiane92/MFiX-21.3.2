...
MODULE SOLVE_GRANULAR_ENERGY_MOD

   use adjust_leq_mod, only: adjust_leq
   use adjust_theta_mod, only: adjust_theta
   ...
   use bc_theta_mod, only: bc_theta
   use calc_nflux_mod, only: calc_nflux
   ...
   use functions, only: fluid_at, wall_at, zmax, funlm
   use geometry, only: ijkmax2, vol
   use init_ab_m_mod, only: init_ab_m
   use kintheory, only: ed_ss_ip
   ...
   use partial_elim_mod, only: partial_elim_ia
   use physprop, only: kth_s, smax, mmax
   use residual, only: num_resid, den_resid
   use residual, only: resid, max_resid, ijk_resid, resid_th
   ...
   use source_ghd_granular_energy_mod, only: source_ghd_granular_energy
   use source_granular_energy_mod, only: source_granular_energy, source_granular_energy_gd, source_granular_energy_ia
   use source_phi_mod, only: source_phi, point_source_phi
   use toleranc, only: zero_ep_s
   use under_relax_mod, only: under_relax_s
   use ur_facs, only: ur_fac
   use usr_src, only: call_usr_source, calc_usr_source, gran_energy
...
END MODULE SOLVE_GRANULAR_ENERGY_MOD
