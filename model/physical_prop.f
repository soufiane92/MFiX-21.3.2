...
! Temperature is now bounded so this error will not occur.
         write(err_msg,2000) IER
         call log_error()
      ENDIF

      return

 2000 FORMAT(/1X,/' From: PHYSICAL_PROP',/' Fatal Error 2000:', &
         ' calc_CpoR reported an invalid temperature: 0x0', I3/,       &
         'See Cp.log for details. Calling MFIX_EXIT.',/1X,/)
...
! Calculate the solids density over all cells.
         IJK_LP: DO IJK = IJKSTART3, IJKEND3
            IF(WALL_AT(IJK)) cycle IJK_LP
            eps_tmp = ep_s(ijk,M)
...
! Local Variables:
!---------------------------------------------------------------------//
! Local aliases for inert species index
      INTEGER :: NN
! Local file values.
      LOGICAL :: lExists
      CHARACTER(LEN=255) :: lFile
