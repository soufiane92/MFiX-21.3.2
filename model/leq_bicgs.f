...
!-------------------------------------------------
      ...
      elseif(PC.eq.'NONE') then
         ...
         IF(DMP_LOG)WRITE (UNIT_LOG,*) &
            'preconditioner option not found'
         CALL LOG_ERROR()
      endif
...
!-----------------------------------------------
      ...
      USE cutcell
      USE functions
      use set_increments_mod, only: shift_dp_array
      use error_manager
...
!-----------------------------------------------
      ...
      interface
         subroutine matvec(vname, var, a_m, r)
            use param
            CHARACTER(LEN=*), INTENT(IN) :: Vname
            DOUBLE PRECISION, INTENT(IN) :: A_m(DIMENSION_3, -3:3)
            DOUBLE PRECISION, INTENT(IN) :: Var(DIMENSION_3)
            DOUBLE PRECISION, INTENT(OUT) :: R(DIMENSION_3)
         end subroutine matvec

         subroutine msolve(vname, b_m, a_m, var, cmethod)
            use param
            CHARACTER(LEN=*), INTENT(IN) :: Vname
...
! A singular matrix was detected.
      IF(IER /= 0) IER_COUNTER=IER_COUNTER+1
      call global_all_sum(IER_COUNTER)
      IF(IER_COUNTER > 0) THEN
         WRITE(ERR_MSG, 1000) VNAME
         call log_warning()
         RETURN
      ENDIF

 1000 FORMAT('Warning: Singular matrix detected when solving for : ',A)
...
!$omp end parallel workshare
      ...
      if(Rnorm0<TOL) Rnorm0=TOL ! JFD: Avoid excessive tolernace criteria when
                                ! first residual is already very small
                                ! Helps run faster at steady state
