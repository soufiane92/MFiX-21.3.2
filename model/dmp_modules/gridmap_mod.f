...
!----------------------------------------------------------------------!
      SUBROUTINE PARTITION(CYC_XLL, CYC_YLL, CYC_ZLL, postmfix)

      implicit none

      logical, intent(in) :: postmfix

! DUMMY Arguments
!---------------------------------------------------------------------//
! Local flags for cyclic boundaries
...
1000     FORMAT(/5X, 'The preconditioner for the linear solver MIGHT NOT be ', /, &
            /5X, 'very efficient with DMP partitions in the y-axis')
         WRITE(ERR_MSG,1000)
         CALL LOG_INFO()
      ENDIF
...
! This works only in the j-direction   <-------------------------
      IF(.NOT.DOMAIN_SIZE_ADJUSTED) THEN
         INQUIRE(FILE='gridmap.dat',EXIST=GRIDMAP_DAT_PRESENT)
         IF(GRIDMAP_DAT_PRESENT .and. .not. POSTMFIX) THEN
            IF(MyPE == PE_IO) THEN
               WRITE(*,*)'Reading gridmap from gridmap.dat...'
...
! The following is general for 1-d or 2-d or 3-d decomposition
...
!----------------------------------------------------------------------!
        SUBROUTINE GRIDMAP_INIT(POSTMFIX)
...
! Amount of load imbalance
      INTEGER :: IMBALANCE
! Theoretical speedup (based on load imbalance)
...
! Set local flags for cyclic boundaries.
      ...
      CALL PARTITION(CYC_XL, CYC_YL, CYC_ZL, postmfix)
...
! indirection arrays
      ...
      if (allocated( imap)) deallocate( imap); allocate( imap( imin3:imax3 ) )
      if (allocated( jmap)) deallocate( jmap); allocate( jmap( jmin3:jmax3 ) )
      if (allocated( kmap)) deallocate( kmap); allocate( kmap( kmin3:kmax3 ) )

      if (allocated( imap_c)) deallocate( imap_c); allocate( imap_c( imin3:imax3 ) )
      if (allocated( jmap_c)) deallocate( jmap_c); allocate( jmap_c( jmin3:jmax3 ) )
      if (allocated( kmap_c)) deallocate( kmap_c); allocate( kmap_c( kmin3:kmax3 ) )
...
! Defining new set of variables to define upper and lower bound of the
...
1000     FORMAT('Parallel load balancing statistics:', 2/, &
            13x, 'Comp. cells', 4X, 'Processor', &
            /3X, 'maximum   ', I11, 4X, I9, &
            /3X, 'minimum   ', I11, 4X, I9, &
            /3X, 'average   ', I11, 6X, '-N/A-', 2/, &
            /3X, 'MAXIMUM speedup (Amdahls Law) = ', A)

         WRITE (ERR_MSG, 1000) &
            maxval(ijksize3_all), maxloc(ijksize3_all), &
...
#ifdef MPI
      COMM = MPI_COMM_WORLD
      CALL SENDRECV_INIT(COMM, idebug=0)
#endif
