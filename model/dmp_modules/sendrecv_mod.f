...
module sendrecv
  ...
  integer, pointer, dimension(:) :: &
       recvproc1, xrecv1, recvijk1, &
       sendproc1, xsend1, sendijk1, &
       recvproc2, xrecv2, recvijk2, &
       sendproc2, xsend2, sendijk2
  ...
  integer, pointer, dimension(:) :: &
       ...
       xrecv,recvproc, recvijk, &
       xsend,sendproc, sendijk
...
contains
  ...
  subroutine sendrecv_init(comm, idebug )
    ...
    integer :: layer,request, source, datatype
    ...
    integer, pointer, dimension(:) :: &
         ...
         ncount, &
         recvproc, xrecv, recvijk,  &
         sendproc, xsend, sendijk
...
9102      format('sendijk2( ', i6,') = ', &
          ...
          do ii=1,nrecv
             ...
             if (lidebug.ge.2) then
                call write_debug(name, 'mpi_recv_init: ii,j1,j2 ', &
                     ii,j1,j2 )
                call write_debug(name, 'icount, source', &
                     icount,source)
             endif

             call MPI_RECV_INIT( drecvbuffer(j1), icount, datatype, &
                  source, 0, comm, request, ierror )
          ...
          do ii=1,nsend
             ...
             if (lidebug.ge.2) then
                call write_debug(name, 'mpi_send_init: ii,j1,j2 ', &
                     ii,j1,j2)
                call write_debug(name, 'icount, dest, ', icount, dest)
             endif

             call MPI_SEND_INIT( dsendbuffer(j1), icount, datatype, &
                  dest, 0, comm, request, ierror )
  ...
  subroutine sendrecv_begin_1d( XX, ilayer, idebug )
    ...
    integer :: layer, datatype, comm, recvsize, sendsize, &
         request, count, source,dest, ierror
          ...
          do ii=1,nrecv
             ...
             if (lidebug.ge.2) then
                call write_debug(name, 'mpi_irecv: ii,j1,j2 ', &
                     ii, j1, j2)
                call write_debug(name, 'count, source', &
                     count,source)
             endif
          ...
          do ii=1,nsend
             ...
             if (lidebug.ge.2) then
                call write_debug(name, 'mpi_isend: ii,j1,j2 ', &
                     ii,j1,j2)
                call write_debug(name, 'count, dest', count, dest)
             endif

             call MPI_ISEND( dsendbuffer(j1), count, datatype, dest, &
                  0, comm, request, ierror )
  ...
  subroutine sendrecv_re_init_after_re_indexing(comm)
