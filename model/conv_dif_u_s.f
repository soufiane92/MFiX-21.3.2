...
MODULE CONV_DIF_U_S_Mod
   ...
   USE dif_u_is_mod, only: dif_u_is
   USE fldvar, only: u_s, v_s, w_s
   USE fun_avg, only: avg_x, avg_x_e, avg_x_h, avg_y_h, avg_z_h
   USE functions, only: bottom_of, east_of, north_of, top_of
   USE functions, only: flow_at_e, wall_at
   USE functions, only: im_of, jm_of, km_of
   USE functions, only: ip_of, jp_of, kp_of
   USE functions, only: north_of, south_of
   USE geometry, only: ayz_u, axz_u, axy_u
   USE geometry, only: ody_n, odx, odz_t, do_k, ox, ox_e
   ...
   USE run, only: def_cor, discretize, ghd_2007, kt_type_enum, momentum_x_eq
