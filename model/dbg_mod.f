...
MODULE dbg
   ...
   use compar, only: iend1, pe_io
   use compar, only: iend1_all, jend1_all, kend1_all
   use compar, only: iend2, jend2, kend2
   use compar, only: iend2_all, jend2_all, kend2_all
   use compar, only: iend3, jend3, kend3
   use compar, only: iend3_all, jend3_all, kend3_all
   use compar, only: iend_all, jend_all, kend_all
   use compar, only: ijksize3
   use compar, only: ijksize3_all
   use compar, only: ijkstart3, ijkend3
   use compar, only: ijkstart3_all, ijkend3_all
   ...
   use compar, only: istart2_all, jstart2_all, kstart2_all
   use compar, only: istart3, jstart3, kstart3
   use compar, only: istart3_all, jstart3_all, kstart3_all
   use compar, only: istart_all, jstart_all, kstart_all
   use compar, only: kstart1, kend1
   use compar, only: mype, numpes
   use exit, only: mfix_exit
   use fldvar
   use functions, only: funijk, fluid_at, is_on_mype_owns
   use functions, only: funijk_io, funijk_proc
   use functions, only: im_of, jm_of, km_of
   use functions, only: ip_of, jp_of, kp_of
   use functions, only: south_of, top_of, west_of, bottom_of, east_of, north_of
   use functions, only: wall_at, funijk_gl
   use funits, only: dmp_log, unit_res
   use geometry, only: ijkmax1, ijkmin1
   use geometry, only: ijkmax2
   use geometry, only: ijkmax2, ijkmax3, do_k
   ...
   use mpi_utility, only: c0, c1, c2, global_all_sum, bcast
   use param, only: dimension_3, dimension_m, north, south, east, west, top, bottom
   use param1, only: undefined_c, undefined_i, zero
   use sendrecv
...
! Unlock dbg routines.
 ...
 1000 FORMAT(/1X,/' From: initExtract',/' Error 1000:',         &
         ' Invalid parameters.  Axis: ',A1,/8x,'LB =',I4,3x,'Min2 =',  &
         I12,/8x,'UB =',I4,3x,'Max2 =',I12,/1x,2/)
...
! for an append file.
 ...
 1000 FORMAT(//1X,/' From: dbg_mod -> arrayExtract_init',/,     &
 ...
 1001 FORMAT(//1X,/' From: dbg_mod -> arrayExtract_init',/,     &
         ' Error 1001: dgbLock is set. Something must have failed.',/  &
         1x,2/)

 1002 FORMAT(//1X,/' From: dbg_mod -> arrayExtract_init',/,     &
         ' Error 1002: Buffer already allocated: ',A,/1x,2/)

 1003 FORMAT(//1X,/' From: dbg_mod -> arrayExtract_init',/,     &
         ' Error 1002: Error opening file: ',A,/1x,2/)
...
! Set local flags based on optional arguments.
      ...
      do J = jLB, jUB
         ...
         IF (DEAD_CELL_AT(I,J,K)) CYCLE  ! skip dead cells
            ...
            dbgIJK = dbg_funijk(I,J,K)
! Only the rank that owns IJK populates lAm and lBm.
! Convert the logical to integer.
...
!      write (fname(8:11),'(i4.4)') myPE
      ...
      IF (AMGDBG .OR. bDist_IO) THEN
         ...
         write(11,"('PE ',I5,': ijkstart3_all = ',I6)") myPE,ijkstart3_all(myPE)
