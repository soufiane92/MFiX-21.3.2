#include "error.inc"
   ...
   ! use error_manager
   ! use leq_msolvert, only: leq_msolvet
   ! use leq_gmres_mod, only: leq_gmres
...
!--------------------------------------------------
      ...
      elseif(PC.eq.'NONE') then
         ...
         IF(DMP_LOG)WRITE (UNIT_LOG,*) 'preconditioner option not found'
         call log_error()
      endif
...
!AIKE PFUPGRADE 091409 Modified ijk to ijk2 to avoid compilation error since PF upgrade
      ...
      interface
         subroutine matvect(vname, var, a_m, r)
            use param
         ...
         subroutine msolvet(vname, b_m, a_m, var, cmethod)
            use param
            CHARACTER(LEN=*), INTENT(IN) :: Vname
...
!     CALL DGTSV( JEND-JSTART+1, 1, DL, DD, EE, BB,  JEND-JSTART+1, INFO )
      ...
      IF (INFO.NE.0) THEN
         IF(DMP_LOG)WRITE (ERR_MSG,*) 'VNAME = ', VNAME
         IF(DMP_LOG)WRITE (ERR_MSG,*) 'DGTSV RETURNS INFO = ', INFO
         call log_error()
      ENDIF
