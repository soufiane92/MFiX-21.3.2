MODULE CALC_RESID_MOD

   use bc, only: bc_defined, bc_plane
   ...
   use fldvar, only: rop_g, rop_s, ep_s
   ...
   use mpi_utility, only: global_all_sum, global_all_max
   use param, only: dimension_3, dimension_m, north, south, east, west, top, bottom, dimension_bc
   use param1, only: ZERO, ONE, UNDEFINED, SMALL_NUMBER, LARGE_NUMBER
   use physprop, only: mmax
   use residual, only: accum_resid_g, accum_resid_s
   use run, only: Added_Mass, dt, m_am, steady_state, debug_resid
...
! Collecting all the information among all the processors -
...
! Determining the maximum among all the processors
...
!$omp  shared (ijkstart3, ijkend3, resid_ijk, i_of, j_of, k_of,m,a_m,b_m,w_m,do_k,u_m,v_m,num,den,ncells,dil_ep_s)
...
! Determine the maximum among all the processors
      call global_all_max(MAX_RESID)
