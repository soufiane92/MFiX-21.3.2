...
MODULE PARSE_LINE_MOD

   USE des_rxns, only: no_of_des_rxns
   USE exit, only: mfix_exit
   USE parse
   USE parse_rxn_mod, only: parse_rxn
   USE rxns
...
! 1) there is an expression to evaluate; @(6.0/2.0) = 3.0
...
! exit MFiX.
         ...
         IF (LEND <= LSTART) THEN
            WRITE (ERR_MSG, 1000) myPE,LINE(LSTART:LMAX)
            call log_error()
         ENDIF
...
! Input line containing an '@(' statement.
      CHARACTER(len=*), INTENT(IN) :: LINE
! Length of of LINE.
      INTEGER LMAX
...
!
      ...
      DO L = LSTART + 2, LEND
         IF (LINE(L:L)=='*' .OR. LINE(L:L)=='/' .OR. LINE(L:L)==END_STR) THEN
            IF (LSUB == 1) THEN
               WRITE (ERR_MSG, 1015) myPE,LINE(LSTART:LEND)
               call log_error()
            ENDIF
      ...
      ELSE IF (LENGTH < 22) THEN
         ...
         IF (LMAX + LDIF > LEN(LINE)) THEN
            WRITE (ERR_MSG, 1020) myPE,LINE(1:80)
            call log_error()
         ENDIF
         DO L = LMAX, LEND + 1, -1
...
!
  900 CONTINUE
      WRITE (ERR_MSG, 1010) myPE, SUB_STR(1:LSUB-1)
      call log_error()
