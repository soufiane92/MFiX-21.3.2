...
! First check if either a constant statwt
      ...
      DO ICV = 1, DIMENSION_IC
         ...
         IF (IC_EP_G(ICV).lt.ONE) THEN
            DO M = MMAX+1, DES_MMAX+MMAX
               ...
               IF(.not.CONST_STATWT.and. ic_ep_s(icv,m).gt.zero) then
...
! that a depth has been set (a value of one implies default setting)
         IF (DZ(1) == ONE) THEN
            ...
            'ZLENGTH in project settings which is not',&
