...
MODULE CHECK_SOLIDS_MODEL_PREREQS_MOD
...
! Flag: Use CGDEM E-L model
      use discretelement, only: CGDEM
...
! Flag: Use DES E-E solids model
      ...
      use run, only: CGP_SOLIDS, CGP_COUNT
...
! Loop over the phases to see what was specified.
      DO M=1, MMAX
         ...
         CASE ('CGP'); CGP_COUNT = CGP_COUNT + 1
         CASE DEFAULT
...
! Set the number of discrete phases.
      DES_MMAX = DEM_COUNT + PIC_COUNT + CGP_COUNT
...
! Set the runtime flags:
      ...
      CGP_SOLIDS = (CGP_COUNT > 0)

      if(CGP_SOLIDS) DEM_SOLIDS = .true.
...
! MPPIC and continuum solids don't mix.
 ...
 1002 FORMAT('Error 1002: MPPIC solids and continuum solids cannot ',&
         'be combined.',/'Please correct the project settings.')
...
! MPPIC and DEM solids don't mix.
 ...
 1003 FORMAT('Error 1003: MPPIC solids and DES solids cannot be ',     &
         'combined.',/'Please correct the project settings.')

! MPPIC and CGP solids don't mix.
      IF(PIC_SOLIDS .AND. CGP_SOLIDS)THEN
         WRITE(ERR_MSG, 1006)
         CALL LOG_ERROR()
      ENDIF
 1006 FORMAT('Error 1006: MPPIC solids and CGPM solids cannot be ',     &
...
! Set the DEM runtime flag.
      DISCRETE_ELEMENT = DEM_SOLIDS .OR. PIC_SOLIDS .OR. CGP_SOLIDS
! Set the MMPIC runtime flag.
      MPPIC = PIC_SOLIDS
! Set the CGDEM runtime flag.
      CGDEM = CGP_SOLIDS
! Set the Hybird flag.
      DES_CONTINUUM_HYBRID = ((DEM_SOLIDS .OR. CGP_SOLIDS).AND. TFM_SOLIDS)
...
! Loop over the phases to see what was specified.
      DO M=1, DIM_M
         ...
         CASE ('DEM', 'CGP'); DEM_MIN = min(M, DEM_MIN)
         END SELECT
      ENDDO
...
END MODULE CHECK_SOLIDS_MODEL_PREREQS_MOD
