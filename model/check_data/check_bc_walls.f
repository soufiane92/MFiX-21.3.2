...
MODULE CHECK_BC_WALLS_MOD

   USE bc, only: par_slip_wall, bc_type_enum
   use error_manager
...
! Input checks for solid phases.
      DO M=1, MTOT_L
         ...
         CASE ('DEM', 'CGP', 'PIC'); CALL CHECK_BC_WALLS_DISCRETE(BCV, M)
         END SELECT
      ENDDO
...
! Total number of species in each phase.
...
! the momentum bc routines
 ...
 1101 FORMAT('Error 1101: Invoking BC_JJ_PS requires GRANULAR_ENERGY', &
         '=.TRUE.',/ 'Please correct the project settings.')
...
! to estimate the phi at r->0
 ...
 1201 FORMAT('Error 1201: KT_TYPE = "GHD" cannot be used with ',&
         ' BC_JJ_PS',/'Please correct the project settings.')
 ...
 1203 FORMAT('Error 1203: JENKINS and BC_JJ_M cannot be used at the',&
         ' same time',/'Please correct the project settings.')
...
! User-Input: solids velocity at wall BCs.
      use bc, only: BC_UW_s, BC_VW_s, BC_WW_s
! User-Input: solids energy eq BCs.
      use bc, only: BC_HW_T_s, BC_C_T_s, BC_Tw_s
! User-Input: solids species eq BCs.
      use bc, only: BC_HW_X_s, BC_XW_s, BC_C_X_s
! Flag for solving energy and species equations
...
! Adiabatic walls.
         IF((BC_HW_T_S(BCV,M) == ZERO) .and. &
            (BC_C_T_S(BCV,M)  == ZERO) .and. &
            (BC_TW_S(BCV,M) == ZERO    .or. &
             BC_TW_S(BCV,M) == UNDEFINED)) THEN

! Unspecified (adiabatic)
         ELSE IF((BC_HW_T_S(BCV,M) == UNDEFINED) .and. &
                 (BC_C_T_S(BCV,M)  == UNDEFINED) .and. &
                 (BC_TW_S(BCV,M) == UNDEFINED)) THEN
...
! Poorly defined wall BC.
 ...
 1100 FORMAT('Error 1100: The energy equation boundary condition ',i3,&
      ' is ill',/'defined for solids ',I2,'. Walls can be adiabatic &
      &or have a',/'specified temperature for DEM. Please correct &
...
! DEM cannot have a species flux at the walls.
      IF(SPECIES_EQ(M)) THEN
         DO NN=1, DIM_N_s
            IF(BC_HW_X_S(BCV,M,nn) /= UNDEFINED .and. &
               BC_HW_X_S(BCV,M,nn) /= ZERO) THEN
               WRITE(ERR_MSG,1101) BCV, trim(iVar('BC_HW_X_s',BCV,M,nn))
               CALL LOG_ERROR()
            ELSEIF(BC_XW_S(BCV,M,nn) /= UNDEFINED .and. &
                   BC_Xw_s(BCV,M,NN) /= ZERO) THEN
               WRITE(ERR_MSG,1101) BCV, trim(iVar('BC_Xw_s',BCV,M,nn))
               CALL LOG_ERROR()
            ELSEIF(BC_C_X_S(BCV,M,nn) /= UNDEFINED .and. &
                   BC_C_X_s(BCV,M,NN) /= ZERO) THEN
