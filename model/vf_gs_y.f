MODULE VF_GS_Y_MOD
   ...
   USE discretelement, only: des_continuum_hybrid, des_oneway_coupled, discrete_element
   USE discretelement, only: f_gds, f_sds, vxf_gds, vxf_sds
   USE drag, only: f_ss, f_gs
   USE fun_avg, only: avg_y
   USE functions, only: ip_at_n, north_of, funlm
