MODULE STIFF_CHEM
   ...
   use fldvar, only: ep_g, ep_s, p_g
   use fldvar, only: ro_g, rop_g, rop_s
   use fldvar, only: t_g, t_s, x_g, x_s
   use functions, only: fluid_at
   use geometry, only: ijkmax2
   use output, only: full_log
   use param1, only: one, zero
   use physprop, only: mmax, nmax, mw_g, mw_mix_g
   use run, only: species_eq, time
   use rxns, only: no_of_rxns
   use sendrecv, only: send_recv
   use stiff_chem_dbg, only: ode_debug_level, reset_ode, write_ode_log, update_ode_old, check_ode_data
   use stiff_chem_maps, only: mapmfixtoode, mapodetomfix
   use stiff_chem_stats, only: update_stiff_chem_stats, write_stiff_chem_stats, init_stiff_chem_stats
...
! Save rates so they can be written in vtu files
