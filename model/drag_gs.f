...
!    also be made to the analogous routine des_drag_gp in the file      C
...
!-----------------------------------------------
      ...
      USE run, only: wen_yu, wen_yu_pcf, koch_hill, koch_hill_pcf, bvk, tpkkv, user_drag, hys

      USE run, only: gao,sarkar,radl,tgs,difelice,difelice_ganser
...
! discrete and continuous solids phases used for the hybrid model
      INTEGER :: MAXM
...
! Force a ZERO drag coefficient for MMS cases.
            ...
            ELSE
               ...
               CASE (GAO)
                  CALL DRAG_GAO(DgA,EPg,Mu,ROg,VREL,DPM,ROs,IJK)

               CASE (SARKAR)
                  CALL DRAG_SARKAR(DgA,EPg,Mu,ROg,VREL,DPM,ROs,IJK)

               CASE (RADL)
                  CALL DRAG_RADL(DgA,EPg,Mu,ROg,VREL,DPM,ROs,IJK)

               CASE (TGS)
                  CALL DRAG_TPKKV(DgA,EPg,Mu,ROPg,VREL,DPM,DPA,phis)

               CASE (DIFELICE)
                  CALL DRAG_DIFELICE(DgA,EPg,Mu,ROPg,VREL,DPM)

               CASE (DIFELICE_GANSER)
                  CALL DRAG_DIFELICE_GANSER(DgA,EPg,Mu,ROPg,VREL,DPM)
...
! Determine drag force coefficient accounting for any underrelaxation
...
!                                                                      C
!  Revision: 1                                                         C
!  Purpose: Minor changes, e.g., fix inconsistency with analogous      C
...
! for very small filtered size, the drag won't be changed:
...
! Note the presence of gas volume fraction in ROPG and factor of 1/2
      ...
      if (phis>0.01D0.and.(F_3*F_3 - 4.0D0*F_1*(F_0-F_2))<0.0D0) print *,'neg sqrt',phis,RE,F_STOKES,F_0,F_1,F_2,F_3
!      if (F_1==0.0D0)  print *,'div zero',F_1
      if (phis>0.01D0) Re_Trans_2 = (F_3 + dsqrt(F_3*F_3 - 4.0D0*F_1 &
           *(F_0-F_2))) / (2.0D0*F_1)
...
! dimensionless drag force F
      DOUBLE PRECISION :: F
! computational optimization array
      DOUBLE PRECISION :: local_array(5)
!-----------------------------------------------
...
! Avoid division by zero if RE=0 in local_array(5)
      IF (RE == ZERO) THEN
         lDgA = ZERO
         RETURN
      ENDIF
...
! (this F_Stokes is /= of Koch_Hill by a factor of ep_g)
      local_array(1)=ONE/EPG/EPG
      local_array(2)=ONE/EPG
      local_array(3)=EPG
      local_array(4)=EPG**2
      local_array(5)=ONE/RE

      F_STOKES = 18D0*Mug*local_array(3)/DPM**2   ! use DPM
      F = 10d0*phis*local_array(1) + local_array(4)*(ONE+1.5d0*DSQRT(phis))
      F = F + 0.017208d0*RE*local_array(1) * &
         (local_array(2) + 3d0*local_array(3)*phis + &
         8.4d0*(local_array(5)**0.343)) / &
         (ONE+10.d0**(3d0*phis)*(local_array(5)**(0.5+2.d0*phis)))
...
!vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvC
!                                                                      C
!  Subroutine: DRAG_TPKKV                                               C
!  Purpose: Calculate the gas-solids drag coefficient                  C
!                                                                      C
!  Literature/Document References:                                     C
!     Tang, Peters, Kuipers, Kreibitzsch, & van der Hoef.              C
!     A new drag correlation from fully resolved simulations of flow   C
!     past monodisperse static arrays of spheres.                      C
!     AIChE J., 61(2), pp.688-698 (2015).                              C
!                                                                      C
!vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvC

   SUBROUTINE DRAG_TPKKV(lDgA,EPg,Mug,ROPg,VREL,&
      DPM,DPA,PHIS)
...
! dimensionless drag force F
      DOUBLE PRECISION :: F
! EPg**2
      DOUBLE PRECISION :: EPg2
! 1/EPg**4
      DOUBLE PRECISION :: oEPg4
!-----------------------------------------------
...
! Eq. (21) of Tang, et al. AIChEJ 61(2) 2015
! (like BVK, this F_Stokes is /= of Koch_Hill by a factor of ep_g)
      F_STOKES = 18D0*Mug*EPg/DPM**2   ! use DPM
      F = 10.0d0*phis/EPg2 + EPg2*(ONE+1.5d0*DSQRT(phis)) &
         + (0.169d0*EPg + 6.44d-2*oEPg4)*RE**0.657d0       &
         + (0.11d0*phis*(1.0d0+phis) - 4.56d-3*oEPg4)*RE

      lDgA = F*F_STOKES

      RETURN
   END SUBROUTINE DRAG_TPKKV
...
!vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvC
!                                                                      C
!  Subroutine: DRAG_GAO                                                C
!  Purpose: Calculate the gas-solids drag coefficient                  C
!                                                                      C
!  Author: X. Gao                                Date: 25-FEB-2021     C
!  Literature/Document References:                                     C
!  Gao, X., Li, T., Sarkar, A., Lu, L., Rogers, W.A. Development and   C
!  Validation of an Enhanced Filtered Drag Model for Simulating        C
!  Gas-Solid Fluidization of Geldart A Particles in All Flow Regimes.  C
!  Chemical Engineering Science, 184, 33-51, 2018.                     C
!                                                                      C
!vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvC

      SUBROUTINE DRAG_GAO(lDgA,EPg,Mug,ROg,VREL,DPM,ROs,IJK)
!-----------------------------------------------
! Modules
!-----------------------------------------------
      USE param
...
! Single sphere drag coefficient
      DOUBLE PRECISION :: C_d
      DOUBLE PRECISION :: phis, h3d
      DOUBLE PRECISION :: velTerminal, vSlipStar, filtersize, filtSizeStar
      DOUBLE PRECISION :: hInf, alphaCoeff, VZero, powerVal
      DOUBLE PRECISION, dimension(18) :: params
      ...
      IF(RE <= 1000.0D0)THEN
         C_d = (24.D0/(RE+0.00000001D0)) * (1.0 + 0.15D0*RE**0.687D0)
      ELSE
         C_d = 0.44D0
      ENDIF

      lDgA = 0.75D0 * C_d * VREL * EPg*ROg * EPg**(-2.65D0) / DPM

      h3d = ZERO
      phis = ONE - EPg
      velTerminal = abs(GRAVITY)* DPM*DPM *(ROs-ROg) / (18.0d0*MUg)
      ...
      filtSizeStar = filtersize / (velTerminal**2/abs(GRAVITY))
      if(filtSizeStar.lt.0.5d0) filtSizeStar = 0.5d0

      params = &
      (/ 0.75597773, 2.73931487, -5.60196497, -1.65853820, 16.70299223, 0.18195034, -0.44145335, -0.01827347, &
      0.28441799,  -1.94357377,  0.22177961,  0.31175890,  -0.15971960,  &
      0.47750002,  0.06279418,  5.13011673,  -0.54535726,  0.67680355  /)

      hInf = 0.95
      alphaCoeff = ( params(1) + params(2)*phis + params(3)*phis**2 + params(4)*phis**3 + params(5)*phis**4) &
              * ( ONE - exp(-300.0d0*phis) )  /  ( ONE + exp(100.0d0*(phis-0.55)) )  &
              * ( ONE + params(7)/filtSizeStar + params(6)/(filtSizeStar**2) )  &
              * ( ONE + params(8)/(vSlipStar**2) )
      VZero = ( params(9) + params(10)*phis ) / ( phis**params(11) + 0.01 )  &
              * ( ONE + params(12)/filtSizeStar + params(13)/(filtSizeStar**2) )
      powerVal = ( params(14) + params(15)*phis + params(16)*phis**2 )  &
              * ( ONE + params(18)/filtSizeStar + params(17)/(filtSizeStar**2) )
...
! multiply by the correction factor
      ...
      RETURN
      END SUBROUTINE DRAG_GAO


!vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvC
!                                                                      C
!  Subroutine: DRAG_SARKAR                                             C
!  Purpose: Calculate the gas-solids drag coefficient                  C
!                                                                      C
!  Author: X. Gao                                Date: 25-FEB-2021     C
!  Literature/Document References:                                     C
!  Sarkar, A., Milioli, F.E., Ozarkar, S., Li, T., Sun, X.,            C
!  Sundaresan, S., 2016. Filtered sub-grid constitutive models for     C
!  fluidized gas-particle flows constructed from 3-D simulations.      C
!  Chem. Eng. Sci., 152, 443-456.                                      C
!                                                                      C
!vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvC

      SUBROUTINE DRAG_SARKAR(lDgA,EPg,Mug,ROg,VREL,DPM,ROs,IJK)
!-----------------------------------------------
! Modules
!-----------------------------------------------
      USE param
...
! Single sphere drag coefficient
      DOUBLE PRECISION :: C_d
      DOUBLE PRECISION :: al, bl, cl, dl, h2_3d
      DOUBLE PRECISION :: phis, h3d
      DOUBLE PRECISION :: velTerminal, vSlipStar, filtersize, filtSizeStar
      DOUBLE PRECISION :: hInf
      DOUBLE PRECISION, dimension(18) :: params
      ...
      al = 0.9506d0
      bl = 0.1708d0
      cl = 0.049d0*(1.d0/filtSizeStar - 1.d0)
      dl = 0.3358d0
      hInf = 0.97d0

      if(vslipstar.le.zero) vslipstar = small_number

      h2_3d = (al + bl/vslipstar)*phis**(cl + dl/vslipstar)*1.0d0
      h3d = min(h2_3d, 0.97)
...
! multiply by the correction factor
      ...
      RETURN
      END SUBROUTINE DRAG_SARKAR


!vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvC
!                                                                      C
!  Subroutine: DRAG_RADL                                               C
!  Purpose: Calculate the gas-solids drag coefficient                  C
!                                                                      C
!  Author: X. Gao                                Date: 25-FEB-2021     C
!  Literature/Document References:                                     C
!  Radl, S., Sundaresan, S., 2014. A drag model for filtered           C
!  Euler-Lagrange simulations of clustered gas-particle suspensions.   C
!   Chem. Eng. Sci., 117, 416-425.                                     C
!                                                                      C
!vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvC

      SUBROUTINE DRAG_RADL(lDgA,EPg,Mug,ROg,VREL,DPM,ROs,IJK)
!-----------------------------------------------
! Modules
!-----------------------------------------------
      USE param
...
! filter size
      DOUBLE PRECISION :: filtersize
      DOUBLE PRECISION :: Froude,L_char
! total solids volume fraction
      DOUBLE PRECISION :: EPs,phis
      DOUBLE PRECISION :: h_d, h_phip, f_filter,a_phip
      ...
      IF(DO_K) THEN
         filtersize = 1.0 * (VOL(IJK)**(ONE/3.0d0))
      ELSE
         filtersize = 1.0 * DSQRT(AXY(IJK))
      ENDIF
...
! particle terminal settling velocity
      vt = GRAVITY*DPM*DPM*(ROs - ROg) / (18.0d0*Mug)
      Froude= vt**2/DPM/GRAVITY
      L_char= vt**2/GRAVITY*Froude**(-2.0d0/3.0d0)

      EPs = ONE - EPg
      phis = ONE -EPg

      IF (EPs .LT. 0.03d0) THEN
         h_phip = 0 + 7.97d0*(EPs-0) + 0*(EPs-0)**2d0 + 0*(EPs-0)**3d0
      ELSEIF (EPs .LT. 0.08d0) THEN
         h_phip = 0.239d0+4.64d0*(EPs-0.03d0)-4.41d0*(EPs-0.03d0)**2d0+253.63d0*(EPs-0.03d0)**3d0
      ELSEIF (EPs .LT. 0.12d0) THEN
         h_phip = 0.492d0 + 6.10d0*(EPs-0.08d0)+33.63d0*(EPs-0.08d0)**2d0-789.6d0*(EPs-0.08d0)**3d0
      ELSEIF (EPs .LT. 0.18d0) THEN
         h_phip = 0.739d0 +5.01d0*(EPs-0.12d0)-61.1d0*(EPs-0.12d0)**2d0+ 310.8d0*(EPs-0.12d0)**3d0
      ELSEIF (EPs .LE. 0.34d0) THEN
         h_phip = 0.887d0+1.03d0*(EPs-0.18d0)-5.17d0*(EPs-0.18d0)**2d0+5.99d0*(EPs-0.18)**3d0
      ELSEIF (EPs .LE. 0.48d0) THEN
         h_phip = 0.943d0-0.170d0*(EPs-0.34d0)-2.29d0*(EPs-0.34d0)**2d0-9.12d0*(EPs-0.34d0)**3d0
      ELSEIF (EPs .LE. 0.55d0) THEN
         h_phip = 0.850d0-1.35d0*(EPs-0.48d0)-6.13d0*(EPs-0.48d0)**2d0-132.6d0*(EPs-0.48d0)**3d0
      ELSEIF (EPs .LE. 0.60d0) THEN
         h_phip = 0.680d0 -2.34d0*(EPs-0.55d0)-225.2d0*(EPs-0.55d0)**2d0 + 0*(EPs-0.55d0)**3d0
      ELSE
         h_phip=ZERO
      ENDIF

      IF (EPs .LT. 0.016d0) THEN
         a_phip = 21.51d0 + 0*(EPs-0) + 0*(EPs-0)**2d0 + 0*(EPs-0)**3d0
      ELSEIF (EPs .LT. 0.10d0) THEN
         a_phip = 1.96d0+29.40d0*(EPs-0) +164.91d0*(EPs-0)**2d0 -1923d0*(EPs-0)**3d0
      ELSEIF (EPs .LT. 0.18d0) THEN
         a_phip = 4.63d0 + 4.68d0*(EPs-0.1d0) -412.04d0*(EPs-0.1d0)**2d0+2254d0*(EPs-0.1d0)**3d0
      ELSEIF (EPs .LT. 0.25d0) THEN
         a_phip = 3.52d0 -17.99d0*(EPs-0.18d0) +128.80d0*(EPs-0.18d0)**2d0-603d0*(EPs-0.18d0)**3d0
      ELSEIF (EPs .LE. 0.40d0) THEN
         a_phip = 2.68d0 -8.82d0*(EPs-0.25d0) +2.18d0*(EPs-0.25d0)**2d0 + 112.33d0*(EPs-0.25d0)**3d0
      ELSE
         a_phip = 1.79d0 + 0*(EPs-0) + 0*(EPs-0)**2d0 + 0*(EPs-0)**3d0
      ENDIF

      f_filter=1.0/(a_phip*L_char/filtersize+1.0d0)
      h_d=1-f_filter*h_phip
      ...
      RETURN
      END SUBROUTINE DRAG_RADL

!vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvC
!                                                                      C
!  Subroutine: DRAG_TGS                                                C
!  Purpose: Calculate the gas-solids drag coefficient                  C
!                                                                      C
!  Author: X. Gao                                Date: 25-FEB-2021     C
!  Literature/Document References:                                     C
!  Tenneti, S., Garg, R., Subramaniam, S., 2011. Drag law for          C
!  monodisperse gas solid systems using particle-resolved direct       C
! numerical simulation of flow past fixed assemblies of spheres.       C
! Int. J. Multiph. Flow, 37(9), 1072-1092.                             C
!                                                                      C
!vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvC

   SUBROUTINE DRAG_TGS(lDgA,EPg,Mug,ROPg,VREL,&
      DPM,DPA,PHIS)
...
! Note the presence of gas volume fraction in ROPG
      ...
      F_ISO=1.0 + 0.15D0*RE**0.687D0
      F = F_ISO/EPg**3d0+5.81d0*phis/EPg**3d0
      F = F + 0.48d0*phis**(1.0d0/3.0d0)/EPg**4d0
      F = F + phis**3d0*RE*(0.95D0+0.61D0*phis**(1.0d0/3.0d0)/(1.0-phis)**2)
      lDgA = F*F_STOKES

      RETURN
      END SUBROUTINE DRAG_TGS

!vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvC
!                                                                      C
!  Subroutine: DRAG_DIFELICE                                           C
!  Purpose: Calculate the gas-solids drag coefficient                  C
!                                                                      C
!  Author: X. Gao                                Date: 25-FEB-2021     C
!  Literature/Document References:                                     C
!  R. Di Felice, The voidage function for fluid-particle interaction   C
!  systems, Int. J. Multiph. Flow 20 (1994) 153-159.                   C
!                                                                      C
!vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvC

      SUBROUTINE DRAG_DIFELICE(lDgA,EPg,Mug,ROPg,VREL,DPM)
...
! Note the presence of gas volume fraction in ROPG
      ...
      lDgA = 0.75D0*C_d*VREL*ROPg*EPg**(1.0-ex) / DPM

      RETURN
      END SUBROUTINE DRAG_DIFELICE

!vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvC
!                                                                      C
!  Subroutine: DRAG_DIFELICE_GANSER                                    C
!  Purpose: Calculate the gas-solids drag coefficient                  C
!                                                                      C
!  Author: X. Gao                                Date: 25-FEB-2021     C
!  Literature/Document References:                                     C
!  R. Di Felice, The voidage function for fluid-particle interaction   C
!  systems, Int. J. Multiph. Flow 20 (1994) 153-159.                   C
!  G.H. Ganser, A rational approach to drag prediction of spherical    C
!  and nonspherical particles, Powder Technol. 77 (1993) 143-152.      C
!                                                                      C
!vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvC

      SUBROUTINE DRAG_DIFELICE_GANSER(lDgA,EPg,Mug,ROPg,VREL,DPM)
...
!-----------------------------------------------
      USE param
      USE param1
      USE run, only: SPHERICITY_DG, REF_LENGTH_DG
      IMPLICIT NONE
!-----------------------------------------------
! Dummy arguments
!-----------------------------------------------
...
! Ganser non-spherical drag coefficient
      dvdD = DPM / REF_LENGTH_DG
      k1=(1.0d0/3.0d0+2.0d0/3.0d0*SPHERICITY_DG**(-0.5d0))**(-1.0d0)-2.25d0*dvdD
      k2= 10.0d0**(1.8148d0*(-log10(SPHERICITY_DG))**0.5743d0)
      ...
      RETURN
      END SUBROUTINE DRAG_DIFELICE_GANSER



END MODULE drag_gs_mod
