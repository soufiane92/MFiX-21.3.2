MODULE WRITE_RES0_MOD

   USE bc
   USE cdist, only: bdist_io
   USE constant, only: c, c_name, c_e, c_f, ep_star, gravity, phi, phi_w, v_ex
   USE funits, only: unit_res
   USE ic
   USE in_binary_512i, only: convert_to_io_i
   USE is
   USE leqsol, only: leq_it, leq_method
   USE machine, only: id_year, id_month, id_day, id_hour, id_minute, id_second
   USE mpi_utility, only: gather, scatter
