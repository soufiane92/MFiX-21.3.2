...
MODULE DESGRID
   ...
   use discretelement, only: dg_pijkprv, dg_pijk, des_pos_new, factor_rlm, max_isize, max_pip, max_isize, pip, max_radius, neighbor_index, des_radius
   use discretelement, only: dimn
   use discretelement, only: xe, yn, zt
   use error_manager, only: err_msg, loglevel_error, loglevel_info, log_message, ival
...
! Confirmation checks
 ...
 1105 FORMAT('Error 1105: Invalid DES grid indices: ',A,/'This is ',   &
         ...
         'partitions',/'for the DES grid with keywords: ',/            &
...
!$    INTEGER omp_get_thread_num
      ...
      LOGICAL :: IS_COARSE_DG_X, IS_COARSE_DG_Y, IS_COARSE_DG_Z
...
!$omp    is_coarse_dg_x, is_coarse_dg_y, is_coarse_dg_z, max_radius) &
!$omp    shared(max_pip,neighbor_index,dg_pijk,NO_K,des_pos_new,dg_pic, factor_RLM,dd,  &
...
!   The desgrid size should not be less than 2*dia*rlm_factor
         ...
         IS_COARSE_DG_X = 1 > 2*MAX_RADIUS*FACTOR_RLM*DG_DXINV
         IS_COARSE_DG_Y = 1 > 2*MAX_RADIUS*FACTOR_RLM*DG_DYINV
         IS_COARSE_DG_Z = 1 > 2*MAX_RADIUS*FACTOR_RLM*DG_DZINV

         if (IS_COARSE_DG_X) then
