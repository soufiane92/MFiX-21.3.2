...
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^C
    ...
    PRIVATE
    PUBLIC :: uni_rno, nor_rno, log_nor_rno
...
! This method generates a pair of independent pseudo-random numbers following
! the standard normal distribution.
! Ref: Peter E. Kloeden Eckhard Platen Henri Schurz, Numerical Solution of SDE
...
!vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv
      SUBROUTINE LOG_NOR_RNO(Y, mu, sigma)
...
!     LOCAL VARIABLES
      ...
      DOUBLE PRECISION, PARAMETER :: norm_mu = 0.0, norm_sigma = 1.0
      INTEGER(8) nsize, i
      ...
      call nor_rno(x, norm_mu, norm_sigma)

      do i = 1, nsize
         y(i) = exp(mu + sigma*x(i))
      enddo

      return

      END SUBROUTINE LOG_NOR_RNO
