...
! Heat transfer correlation keyword
! Default [RANZ_1952]
      CHARACTER(LEN=24) :: DES_CONV_CORR

      INTEGER :: DES_CONV_CORR_ENUM
      INTEGER, PARAMETER :: RANZ_1952 = 0
      INTEGER, PARAMETER :: WAKAO     = 1
      INTEGER, PARAMETER :: GUNN      = 2
      INTEGER, PARAMETER :: Tavassoli = 3
...
! the fluid lens that surrounds the particle for particle-fluid-particle
! conduction.  Default [ 0.2 ]
      DOUBLE PRECISION :: FLPC

! Minimum separation distance between the surface of two contacting
! particles. This value is used to remove the singularity that the
...
! Convert to SI units if needed.
      ...
      ELSE
! If a constant value specific heat has been assigned to the particle, use this value.
