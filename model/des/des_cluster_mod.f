...
!-----------------------------------------------
   ...
   use desmpi_wrapper, only: des_mpi_gatherv, des_mpi_scatterv, global_sum, des_mpi_barrier
   use discretelement, only: iglobal_id, pijk, des_pos_new, des_vel_new, des_radius, postcluster
   use error_manager, only: loglevel_error, log_message
   use fldvar, only: ep_g, u_g, v_g, w_g, t_g, t_go, rop_go, ro_g
   use functions, only: is_ghost
   use geometry, only: do_k, no_k
   use param1, only: large_number, undefined, zero
