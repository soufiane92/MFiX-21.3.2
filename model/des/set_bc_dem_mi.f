...
MODULE SET_BC_DEM_MI_MOD

   use error_manager
   use layout_mi_dem_mod, only: layout_mi_dem
...
! Average particle volume given a size distribution
...
! Determine if the inlet is mono or polydisperse
         DO M=1, MMAX + DES_MMAX
            ...
            if(CGDEM.and.cgp_scaling_method(m)==2) max_dia = max(max_dia,cgp_d_p0(m))
         ENDDO
...
! second for each solid phase present at the boundary.
            if(BC_PSD_TYPE(BCV,M) .eq. 'MONO') then
              p_vol = (pi*d_p0(m)**3.0D0)/6.0d0
            elseif(CGDEM.and.cgp_scaling_method(m)==2) then
               p_vol = (pi*cgp_d_p0(m)**3.0D0)/6.0d0
            else
              p_vol = mean_volume(2, BCV, M)   ! 2 indicates BC (1 will indicate IC)
            end if
! Compute global mean of p_vol for all processes
...
! Otherwise use the volumetric flow rate if defined.
            ...
            ELSEIF(BC_VOLFLOW_S(BCV,M) /= UNDEFINED) THEN
               NPMpSEC(M) = BC_VOLFLOW_s(BCV,M) / p_vol
...
! Inject one particle every PI_FACTOR solids time steps.
         ...
         ELSEIF(NPpDT .LT. 1.0)THEN
            PI_COUNT(BCV_I) = 1
            PI_FACTOR(BCV_I) = real(1.d0/NPpDT) !llu more accurate
! Inject PI_COUNT particles every soilds time step.
         ELSE
            PI_COUNT(BCV_I) = CEILING(real(NPpDT))
            PI_FACTOR(BCV_I) = 1 ! llu, should be 1.xxx
         ENDIF
...
! uniform inlet velocity.
 ...
 1200 FORMAT('Error 1200: Unable to impose a uniform solids velocity ',&
         ...
         2/5X,'Max Inlet Velocity',':',1X,ES11.4,/5x,'Total BC_EP_s Error:',ES11.4)
...
! with an updated value.
 ...
 1000 FORMAT(2/,2X,'For mass inlet BC: ', I3,/,&
         ...
         4X,'PI_FACTOR = ', ES15.8' PI_COUNT = ', I5,/,&
