MODULE WRITE_RES1_DES
   ...
   use desmpi
   use discretelement, only: MAX_PIP, PIP, iGHOST_CNT, iGlobal_ID
...
! not collected and should be on rank 0.                               !
!``````````````````````````````````````````````````````````````````````!
...
!                                                                      !
! Purpose: Write an array of double precision values to RES file. Note !
