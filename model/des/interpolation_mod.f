...
! only needs to return the IJK indices of the cells and the interpolation
...
! increment the loop counter
               LC = LC + 1
! Shift loop indices to the correct values
               II = IW + I-1
               JJ = JS + J-1
               KK = KB + K-1
...
! Assign a dummy value for the field variable
...
!-----------------------------------------------
    ...
    CASE('csi')
       ...
       !The definitions for zetacsi are true only for a structured grid
       !if (order.eq.4) then
