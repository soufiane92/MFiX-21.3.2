...
! NREL_CU_OPT sorting,swappping routines
      integer :: valid_ind
      double precision :: temp_db
      double precision :: rops_inv
...
!$omp         NINDX, NP, M, WEIGHT_FT, I1, I2, J1, J2, K1, K2, &
!$omp         IDIM, IJK2, NORM_FACTOR, RESID_ROPS, RESID_VEL, &
!$omp         COUNT_NODES_OUTSIDE, TEMP1) &
!$omp private(valid_ind) &
!$omp shared(ncells_valid_pinc,valid_fluid_indices)
...
!$omp        MMAX, DES_ROPS_NODE, DES_VEL_NODE, optflag1) &
!$omp private(I, J, K, IJK, M, II, JJ, KK, IJK2, DES_ROP_DENSITY, &
!$omp         DES_VEL_DENSITY) &
!$omp reduction(+:ROP_S, U_S, V_S, W_S) &
!$omp private(temp_db) &
!$omp shared(fluid_at_mask2,do_k_mask,vol_surr_inv)
      DO K = KSTART2, KEND1
      DO J = JSTART2, JEND1
      DO I = ISTART2, IEND1
...
! looping over stencil points (NODE VALUES)
         DO M = MMAX+1, MMAX_TOT
           if (optflag1.eq.1) then
             DES_ROP_DENSITY = DES_ROPS_NODE(IJK, M)*VOL_SURR_INV(IJK)
             DES_VEL_DENSITY(:) = DES_VEL_NODE(IJK, :, M)*VOL_SURR_INV(IJK)
           else
            ...
            DO II = I, I+1
               ...
               if (optflag1.eq.1) then
                 TEMP_DB = VOL(IJK2)*fluid_at_mask2(II-ISTART2+1,JJ-JSTART2+1,KK-KSTART2+1)
                 ROP_S(IJK2, M) = ROP_S(IJK2, M) + DES_ROP_DENSITY*TEMP_DB
                 U_S(IJK2, M) = U_S(IJK2, M) + DES_VEL_DENSITY(1)*TEMP_DB
                 V_S(IJK2, M) = V_S(IJK2, M) + DES_VEL_DENSITY(2)*TEMP_DB
                 W_S(IJK2, M) = W_S(IJK2, M) + DES_VEL_DENSITY(3)*TEMP_DB*do_k_mask
               else
...
!$omp        V_S, W_S, VOL, optflag1) &
!$omp private(valid_ind,rops_inv) &
!$omp shared(ncells_valid,valid_fluid_indices,do_k_mask,start_index,end_index)
      ...
      DO do_index = start_index, end_index
         ...
         DO M = MMAX+1, MMAX_TOT
            IF(ROP_S(IJK, M).GT.ZERO) THEN
               if (optflag1.eq.1) then
                  ROPS_INV = ONE/ROP_S(IJK,M)
                  U_S(IJK, M) = U_S(IJK,M)*ROPS_INV
                  V_S(IJK, M) = V_S(IJK,M)*ROPS_INV
                  W_S(IJK, M) = W_S(IJK,M)*ROPS_INV*do_k_mask
               else
...
!-----------------------------------------------
  ...
  END MODULE COMP_MEAN_FIELDS0_MOD
