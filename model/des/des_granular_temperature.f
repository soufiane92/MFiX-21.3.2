MODULE DES_GRANULAR_TEMPERATURE_MOD

   use compar, only: ijkstart3, ijkend3
   use discretelement, only: omega_new, des_vel_new, des_pos_new, des_radius, pijk, pmass
   use discretelement, only: max_pip, tot_par, des_vel_avg, global_gran_energy, global_gran_temp
   use discretelement, only: des_mmax, pip, des_ke, des_rote, des_pe, dimn, ighost_cnt, yn, grav, bed_height
   use fldvar, only: u_s, v_s, w_s, theta_m, rop_s, ep_s
   use functions, only: is_nonexistent, is_ghost, is_entering_ghost, is_exiting_ghost, fluid_at
   use geometry, only: vol, do_k, no_k
...
!           accommodate ghost cells  (Pradeep G.)                      C
!                                                                      C
!                                                                      C
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^C
...
END MODULE DES_GRANULAR_TEMPERATURE_MOD
