...
MODULE write_des_data_mod

   use cdist, only: bdist_io
   use compar, only: ijkstart3, ijkend3, pe_io, mype, numpes
   use des_granular_temperature_mod, only: calc_des_bedheight, des_granular_temperature
   use des_rxns, only: des_x_s
   use des_thermo, only: des_t_s
   use desmpi, only: dprocbuf, drootbuf, iprocbuf, irootbuf, igath_sendcnt, igathercnts, idispls
   use desmpi, only: idispls, igathercnts
   use discretelement, only: des_pos_new, des_vel_new, des_usr_var, des_output_type
   use discretelement, only: des_radius, omega_new, ro_sol, tecplot_findex, pip, iglobal_id, max_pip
   use discretelement, only: des_usr_var, des_usr_var_size, bed_height, des_mmax, ighost_cnt, pinc
   use discretelement, only: s_time, particle_orientation, orientation
   use discretelement, only: use_cohesion, postcohesive, des_calc_bedheight
   use discretelement, only: CGDEM, DES_CGP_RPR, DES_CGP_STW
   use error_manager, only: loglevel_error, log_message, ival, ivar
   use fldvar, only: theta_m
   use functions, only: is_nonexistent, fluid_at, is_ghost, is_entering_ghost, is_exiting_ghost
   ...
   use run, only: any_species_eq, energy_eq, run_name, run_type, time
...
!----------------------------------------------------------------------/
      ...
      IF(CGDEM) THEN
         CALL VTP_WRITE_DATA('PhysicalRadius', DES_CGP_RPR)
         CALL VTP_WRITE_DATA('StatisticWeight', DES_CGP_STW)
      ENDIF
...
!           large simulations. Hence separate files are written for
!           each instances
!  Author : Pradeep G.
!
!
...
! set the file name and unit number and open file
      ...
      des_eps = 2200
      if (bdist_io) then
         write(fname_data,'(A,"_DES_DATA_",I4.4,"_",I4.4,".dat")') trim(run_name),tecplot_findex,mype
         write(fname_extra,'(A,"_DES_EXTRA_",I4.4,"_",I4.4,".dat")') trim(run_name),tecplot_findex,mype
         write(fname_eps,'(A,"_DES_EPS_",I4.4,"_",I4.4,".dat")') trim(run_name),tecplot_findex,mype
...
! write data in distributed mode or as a single file
      if(bdist_io) then
         pc = 1
         do l = 1,max_pip
            if(pc.gt.pip) exit
...
! header information.
         ...
         ELSE
! To prevent overwriting existing files accidentally, exit if the file
