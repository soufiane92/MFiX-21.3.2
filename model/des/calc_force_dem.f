...
! Force chain data
      ...
      INTEGER :: old_size,new_size ! New Force chaine array size when resizing (growing) arrays
...
!$omp    fchainc,fchainc_old,fchain_midpoint,fchain_fn,fchain_length,  &
!$omp    fchain_fn_mag,old_size,new_size)&
...
!Rolling friction
            if(MEW_R>ZERO) then
               ...
               if(OMEGA_MAG > ZERO) then
                  RFTOW = -MEW_R*KN_DES * OVERLAP_N*OMEGA_NEW(LL,:)/OMEGA_MAG
                  TOW(LL,:) = TOW(LL,:) + RFTOW
               endif
               !particle i
               OMEGA_MAG = sqrt(dot_product(OMEGA_NEW(I,:),OMEGA_NEW(I,:)))
               if(OMEGA_MAG > ZERO) then
                  RFTOW = -MEW_R*KN_DES * OVERLAP_N*OMEGA_NEW(I,:)/OMEGA_MAG
                  TOW(I,:) = TOW(I,:) + RFTOW
               endif
            endif
