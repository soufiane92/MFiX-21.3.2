...
! check to see if particles are within lens thickness from each other
...
!          set to a different value with keyword DES_MIN_COND_DIST.    !
!                                                                      !
...
! Unphysical inputs, set EVAL_H_PFP to zero and return      
      IF(RLENS<=ONE.OR.OLAP>ONE) THEN
         EVAL_H_PFP = ZERO
         RETURN
      ENDIF
      ...
      ELSE
         WRITE(*,*)'ERROR: Extremely excessive overlap (Olap > Diam)'
...
! Cannot take log of a negative number, set EVAL_H_PFP to zero and return      
      IF(ONE-OLAP-sqrt(ONE-Rout**2)<=ZERO) THEN
         EVAL_H_PFP = ZERO
         RETURN
      ENDIF
...
!                                                                      !
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^!
      FUNCTION DES_CONDUCTION_WALL(I, OLAP, K_sol, K_wall, K_gas, TWall, &
      TPart, Rpart, RLens, M)
...
! See Lu et al. 2017, IJHMT, 111, 723-735
! Same to p-p conduction

      if(CGDEM) Q_pfw = Q_pfw*DES_RADIUS(I)/DES_CGP_RPR(I)
      if(CGDEM) Q_pw  = Q_pw * DES_CGP_STW(I)**(17.0D0/12.0D0)
