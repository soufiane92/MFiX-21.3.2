...
! Fixed array sizes in the I/J/K direction
      ...
      use check_cell_movement_mod, only: check_cell_movement
...
! skipping particles that do not exist
         ...
         ELSE
            ...
            IF(DEM_SOLIDS) CALL DELETE_PARTICLE(L)
            IF(PIC_SOLIDS) CALL DELETE_PARCEL(L)
         ENDIF
...
! boundaries as well as updates ghost particles information
      if(.not.MPPIC) CALL DES_PAR_EXCHANGE
...
! unclear why this needs to be called again.
   ...
   END MODULE PARTICLES_IN_CELL_MOD
