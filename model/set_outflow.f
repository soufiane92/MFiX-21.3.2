MODULE SET_OUTFLOW_MOD
   use bc, only: p_inflow, bc_type_enum
CONTAINS
...
! such that it is positive when exiting the domain
...
! the PI call must be made first as it may overwrite the existing
! boundary values depending on flow direction
...
! This provides a mechanism to dynamically update P_g in a mass flow
! cell based on the neighbor fluid cell after the pressure corrections
! have been applied to the field.... (note the pressure correction in
! the mass flow cell itself will be zero; but not necessarily in the
! neighboring fluid cell...). That said, the actual value of P in a mass
! flow cell shouldn't really matter as this is not a cell whose value
! enters the equations anywhere...  So in that sense, this is mere
! formality.
! TODO: This update should come before calculations to density then
! this could be removed here along with mw/ro_g..; however, then the
! update to scalars for a PI boundary should also come sooner..
      IF (BC_TYPE_ENUM(BCV) /= P_OUTFLOW .AND. &
          BC_TYPE_ENUM(BCV) /= P_INFLOW) P_G(IJK) = P_G(FIJK)

! This will update the mw at the boundary. Again this shouldn't
! be any different than what would be calculated in an earlier
! call to physical properties/density.  However, a PI boundary
! does get updated in this routine (including species fractions)
! depending on flow direction.  So for that case it could matter.
      MW_MIX_G(IJK) = MW_MIX_G(FIJK)

! Pressure at the boundary has only now been updated to reflect flow
! stream conditions in mass flow boundaries (MO, O). Thus for those
! boundaries the density (via pressure) may have slightly changed...
! although for such boundaries it is unclear whether any of this
! should actually matter..
! Note: it seems that this may effectively have served as a sort of
! initialization for a new run when using a MO, O boundary... where
! p_g at boundary had not otherwise been set yet via solve_pp_g.
! However,  given recent changes in the set_bc0 routines it could
! probably be removed other than as a formality..
...
!  cell.                                                               C
!  At minimum, the value of ep_g/rop_s(ep_s) is needed by pressure     C
!  inflow/outflow boundaries to assist in evaluating a drag            C
!  coefficient for those boundaries which is in turn used to           C
!  evaluate the drag force at those faces for the momentum eqn.        C
!  This boundary cell value is assigned here based on either a user    C
!  specified value or assigned by the neighboring fluid cell.          C
!  Mass outflow/outflow boundaries should not need such a value but    C
!  MFIX check data routines (check_data_20) looks to ensure ep_g and   C
!  rop_s are defined in all non wall cells.                            C
!                                                                      C
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^C
...
! for the case of a udf density bc_rop_s will be undefined, however,
! bc_ep_s is requested in certain circumstances for MO/MI
...
! address gas phase
      IF (RVEL_G < 0) THEN   ! inflow (apply BC)
         ...
         IF (NScalar > 0) THEN
            ...
            ENDDO
         ENDIF
! this condition should likely already be the case given their
! treatment in bc_phi
