...
MODULE SET_INCREMENTS_MOD

   USE bc, only: IJK_P_G
   USE cdist, only: bdist_io
   USE cutcell
   USE discretelement, only: DES_MMAX
   USE energy, only: gama_rg, t_rg
   USE energy, only: gama_rs, t_rs
   USE error_manager, only: err_msg, log_message, loglevel_error, ival
   USE fldvar
   USE funits, only: dmp_log
   USE mpi_utility, only: GLOBAL_ALL_SUM
   USE mpi_utility, only: allgather_1i
   USE parallel
   USE param, only: dimension_3
   USE param1, only: undefined, undefined_i
   USE pgcor, only: PHASE_4_P_G
   USE physprop, only: mmax, c_pg, k_g, dif_g, mu_g
   USE physprop, only: mu_g0, ro_g0
   USE physprop, only: nmax, c_ps, k_s, dif_s, mw_mix_g
   USE pscor, only: PHASE_4_P_S
   USE scalars, only: nscalar
   USE sendrecv
   USE set_index1a_mod, only: set_index1a
...
! Step 0: Identify dead cells

      IF(MyPE == PE_IO) WRITE(*,*)' Re-indexing: Identifying dead cells ...'
...
! GARG_2012 does nor work with re_indexing, This is currently flagged
! as a fatal error, and there is no need to shift the arrays
!      CALL SHIFT_LOG_ARRAY(SCALAR_NODE_ATWALL(1:DIMENSION_3),.FALSE.)
...
!  INSERT NEW SEND_RECV INIT HERE
   ...
   call sendrecv_re_init_after_re_indexing(comm)
...
! ALLOC increment arrays and report an allocation errors.
      ...
      if (allocated(J_OF)) deallocate(J_OF); allocate( J_OF (DIMENSION_3))
      if (allocated(K_OF)) deallocate(K_OF); allocate( K_OF (DIMENSION_3))
      IF(IER /= 0) goto 500

      if (allocated(Im1)) deallocate(Im1); allocate( Im1 (0:DIMENSION_I))
      if (allocated(Ip1)) deallocate(Ip1); allocate( Ip1 (0:DIMENSION_I))
      IF(IER /= 0) goto 500

      if (allocated(Jm1)) deallocate(Jm1); allocate( Jm1 (0:DIMENSION_J))
      if (allocated(Jp1)) deallocate(Jp1); allocate( Jp1 (0:DIMENSION_J))
      IF(IER /= 0) goto 500

      if (allocated(Km1)) deallocate(Km1); allocate( Km1 (0:DIMENSION_K))
      if (allocated(Kp1)) deallocate(Kp1); allocate( Kp1 (0:DIMENSION_K))
      IF(IER /= 0) goto 500

      if (allocated(STORE_LM)) deallocate(STORE_LM); allocate( STORE_LM (DIMENSION_M, DIMENSION_M))
      if (allocated(CELL_CLASS)) deallocate(CELL_CLASS); allocate( CELL_CLASS (DIMENSION_3))
      IF(IER /= 0) goto 500
