...
!                      particle size distribution parameters at boundary region
      CHARACTER(64) :: BC_PSD_TYPE(DIMENSION_BC, DIM_M)
      DOUBLE PRECISION :: BC_PSD_MEAN_DP(DIMENSION_BC, DIM_M)
      DOUBLE PRECISION :: BC_PSD_STDEV(DIMENSION_BC, DIM_M)
      DOUBLE PRECISION :: BC_PSD_MAX_DP(DIMENSION_BC, DIM_M)
      DOUBLE PRECISION :: BC_PSD_MIN_DP(DIMENSION_BC, DIM_M)
      DOUBLE PRECISION :: BC_PSD_MU(DIMENSION_BC, DIM_M)
      DOUBLE PRECISION :: BC_PSD_SIGMA(DIMENSION_BC, DIM_M)
!                      Starting and ending time of the mass inflow boundary
      DOUBLE PRECISION :: BC_MI_START_TIME(DIMENSION_BC)
      DOUBLE PRECISION :: BC_MI_END_TIME (DIMENSION_BC)
!
...
!                      FLAG to specify if a BC (currently a Pressure outlet
!                      or Mass inlet, acts as an outlet for all solid phases
!                      in discrete implementation or not. For example,
!                      this can be used to prevent particles from exiting a PO
!                      BC, or used to let particles exit an MI BC, where only
!                      gas is coming in.
      LOGICAL          BC_PO_APPLY_TO_DES (DIMENSION_BC)

!                      FLAG to specify if a BC (currently only Pressure outlet BC)
!                      can also act as an inlet for the DEM particles.
!                      in discrete implementation or not. For example, this can
!                      used to drop particles from a top gas outlet.
      LOGICAL          BC_MI_APPLY_TO_DES (DIMENSION_BC)
...
!
!                      Coefficient in granular temperature boundary condition -- solids
...
!                                                                       !
!-----------------------------------------------------------------------!
function bc_ro_s(lbcv,lphase) result(lROs)

  use run, only: solve_ros
  use physprop, only: ro_s0, x_s0, inert_species
  use eos, only: eoss, eoss0
  ...
  double precision :: lROs, lROs0

  ! Density of particle at BC
  if(solve_ros(lphase)) then
     lROs0 = eoss0(lphase)
     inert = inert_species(lphase)
     lROs = eoss(lROs0, x_s0(lphase,inert), &
          bc_x_s(lbcv,lphase,inert))
  else
     lROs = ro_s0(lphase)
  endif

end function bc_ro_s
