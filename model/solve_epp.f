MODULE SOLVE_EPP_MOD

   use adjust_leq_mod, only: adjust_leq
   use ambm, only: a_m, b_m, lock_ambm, unlock_ambm
   use calc_resid_mod, only: calc_resid_pp
   use conv_source_epp_mod, only: conv_source_epp, point_source_epp
   ...
   use residual, only: resid, max_resid, ijk_resid, resid_p
   use solve_lin_eq_mod, only: solve_lin_eq
   use toleranc, only: epp_den
   use usr_src, only: call_usr_source, calc_usr_source, solids_correction
...
! and use this to form normalization factor
         ...
         NORMSloc = RESID(RESID_P,M)/EPP_DEN
      ENDIF
