...
MODULE calc_trd_g_mod

   use bc, only: bc_uw_g, bc_vw_g, bc_ww_g, bc_hw_g
   use bc, only: none, cg_nsw, cg_fsw, cg_psw, cg_mi, cg_po, bc_type_enum
   use compar, only: ijkstart3, ijkend3
   use cutcell, only: blocked_u_cell_at, blocked_v_cell_at, blocked_w_cell_at
   use cutcell, only: cut_cell_at, wall_u_at, wall_v_at, wall_w_at, noc_trdg, bc_id
   use cutcell, only: x_u, y_u, z_u
   use cutcell, only: x_v, y_v, z_v
   use cutcell, only: x_w, y_w, z_w
   use fldvar, only: u_g, v_g, w_g
   use fun_avg, only: avg_x, avg_x_e
   use fun_avg, only: avg_y, avg_y_n
   use fun_avg, only: avg_z, avg_z_t
   use functions, only: im_of, jm_of, km_of, flow_at
   use functions, only: ip_of, jp_of, kp_of, wall_at
   use geometry, only: cylindrical, no_k, x_e, ox
