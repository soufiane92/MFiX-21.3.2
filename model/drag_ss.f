...
! should never hit this
               WRITE (ERR_MSG, '(A)') 'DRAG_SS'
               WRITE (ERR_MSG, '(A,A)') 'Unknown KT_TYPE: ', KT_TYPE
            call log_error()
            END SELECT
         ELSE
            CALL DRAG_SS_SYAM(L, M)
...
!  Purpose: Compute collisional momentum source terms between solids   !
!  phase M and solids phase L using Iddir Arastoopour (2005) kinetic   !
!  theory model that is proportional to the relative velocity between  !
