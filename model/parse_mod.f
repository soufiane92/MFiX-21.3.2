...
! is allocated (in part or fully) to this phase. Flag error and exit.
            ELSEIF(.NOT.pMap(M) .AND. lFDH(M) .NE. UNDEFINED) THEN
                  write(err_msg, "(/1X,70('*')/' From: From: setReaction:',/       &
                     ...
                     ' input formats',/' and correct the data file.',/1X,70('*')/)") trim(lName)
               call log_error()
            ENDIF
         ENDDO
...
! Construct the error flag.
            ...
            write(ERR_MSG,"(/1X,70('*')/' From: From: setReaction --> checkSplit',/   &
               ...
               trim(lName), 'Illegal', trim(lChemEq), trim(Flag)
            call log_error()
      ENDIF
! If there are more than one operator, flag error and exit.
      IF(hArr /= 0 .AND. hEqs /= 0) THEN
            ...
            write(ERR_MSG, "(/1X,70('*')/' From: From: setReaction --> checkSplit',/   &
               ...
               ' Please refer to the Readme file for chemical equation', &
               ' input formats',/' and correct the data file.',/1X,70('*')/)") &
               trim(lName), 'Too many', trim(lChemEq), trim(Flag)
            call log_error()
...
! Construct the error flag.
            FLAG = setFlag(20, hArr)
               write(err_msg,"(/1X,70('*')/' From: From: setReaction --> checkSplit',/   &
                  ...
                  ' Please refer to the Readme file for chemical equation', &
                  ' input formats',/' and correct the data file.',/1X,70('*')/)") &
                  trim(lName), 'Missing the tail; -->', trim(lChemEq), trim(Flag)
               call log_error()
         ELSEIF(tArr > hArr) THEN
               ...
               write(err_msg,"(/1X,70('*')/' From: From: setReaction --> checkSplit',/   &
                  ...
                  ' Please refer to the Readme file for chemical equation', &
                  ' input formats',/' and correct the data file.',/1X,70('*')/)") &
                  trim(lName), 'Arror head precedes the tail; -->', trim(lChemEq), trim(Flag)
               call log_error()
         ELSE
...
! and sent out an error message.
      ELSE
         ...
         write(err_msg,"(/1X,70('*')/' From: From: setReaction --> checkSplit',/  &
            ...
            /' Chemical Equation: ',A,/1X, A/, &
            ' Please refer to the Readme file for chemical equation', &
            ' input formats',/' and correct the data file.',/1X,70('*')/)") trim(lName), trim(lChemEq), trim(Flag)
...
! Make sure that there isn't another asterisk further down the string.
         ...
         IF(aPOS /= a2POS) THEN
               ...
               write(err_msg,"(/1X,70('*')/' From: From: setReaction -->',               &
                  ...
                  /' Chemical Equation: ',A,/1X, A/, &
                  ' Please refer to the Readme file for chemical equation', &
                  ' input formats',/' and correct the data file.',/1X,70('*')/)") trim(lName), 'Too many', trim(lChemEq), trim(Flag)
...
! Construct the error flag.
               ...
               write(err_msg,"(/1X,70('*')/' From: From: setReaction -->',      &
                  ' splitAliasAndCoeff',/' Message: Error determining the',     &
                  ' stoichiometric coefficient in the',/' chemical equation',   &
                  ' for reaction ',A,'.', /' Chemical Equation: ',A,/1X, A/,    &
                  ' Please refer to the Readme file for chemical equation',     &
                  ' input formats',/' and correct the data file.',/1X,70('*')/)") trim(lName), trim(lChemEq), trim(Flag)
               call log_error()
            ENDIF
...
! No matching species was located. Flag an error and exit.
         ...
         write(ERR_MSG,1000) trim(lAlias(L)), trim(lName)
         write(ERR_MSG,1010) trim(lChemEq), trim(Flag)
 ...
 1001 FORMAT(/' Please refer to the Readme file for chemical equation',&
         ' input formats',/' and correct the data file.',/1X/)
