...
MODULE SET_FLAGS_MOD

   USE compar, only: dead_cell_at
   USE compar, only: ijkstart3, ijkend3
   USE compar, only: istart3, iend3, jstart3, jend3, kstart3, kend3
   USE compar, only: mype, numPEs, pe_io, root
   USE error_manager, only: loglevel_error, log_message
   USE functions, only: funijk, is_on_mype_plus2layers, bound_funijk, wall_at
   USE functions, only: im_of, ip_of, jm_of, jp_of, km_of, kp_of, cyclic_at, fluid_at
   USE funits, only: dmp_log, unit_log
   USE geometry, only: do_i, do_j, do_k, no_k, icbc_flag
   USE geometry, only: flag_n, flag_t, flag_e, flag
   USE geometry, only: imax2, jmax2, kmax2
   USE geometry, only: imax3, jmax3, kmax3, ijkmax3
   USE indices, only: i_of, j_of, k_of
   USE is, only: dimension_is
   USE is, only: is_defined, is_plane, is_type
   USE is, only: is_i_e, is_j_n, is_k_b
   USE is, only: is_i_w, is_j_s, is_k_t
   USE is, only: is_x_e, is_y_n, is_z_b
   USE is, only: is_x_w, is_y_s, is_z_t
   USE mpi_utility, only: scatter, gather
