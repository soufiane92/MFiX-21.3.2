MODULE CG_SOURCE_W_S_MOD
   ...
   USE geometry
   USE indices
   USE param, only: north, south, east, west, bottom, top
   USE param, only: dimension_3, dimension_m
   USE param1, only: zero, one, undefined
   USE param1, only: zero, undefined
   USE physprop, only: cv  ! virtual (added) mass coefficient Cv
   USE physprop, only: smax, mmax  ! number of solids phases
   USE quadric
   USE run
...
! See comment on CG_MI in CG_set_bc0.f
!              CASE (CG_MI)
!                 A_M(IJK,east,M) = ZERO
!                 A_M(IJK,west,M) = ZERO
!                 A_M(IJK,north,M) = ZERO
