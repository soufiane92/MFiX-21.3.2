      ...
      SELECT CASE (TYPE_OF_CELL)
         CASE('SCALAR')
            ...
            IF(.NOT.CUT_CELL_AT(IJK)) THEN
               WRITE(ERR_MSG,*)' ERROR IN SUBROUTINE GET_DEL_H:'//&
               ' SCALAR CELL',IJK,' IS NOT A CUT CELL'//&
               ' MFiX will exit now.'
               call log_error()
            ENDIF
         ...
         CASE('U_MOMENTUM')
            ...
            IF(.NOT.CUT_U_CELL_AT(IJK)) THEN
               WRITE(ERR_MSG,*)' ERROR IN SUBROUTINE GET_DEL_H:'//&
                  ' U-MOMENTUM CELL',IJK,' IS NOT A CUT CELL'//&
                  ' MFiX will exit now.'
               call log_error()
            ENDIF
         ...
         CASE('V_MOMENTUM')
            ...
            IF(.NOT.CUT_V_CELL_AT(IJK)) THEN
               WRITE(ERR_MSG,*)' ERROR IN SUBROUTINE GET_DEL_H:'//&
                  ' V-MOMENTUM CELL',IJK,' IS NOT A CUT CELL'//&
                  ' MFiX will exit now.'
               call log_error()
            ENDIF
         ...
         CASE('W_MOMENTUM')
            ...
            IF(.NOT.CUT_W_CELL_AT(IJK)) THEN
               WRITE(ERR_MSG,*)' ERROR IN SUBROUTINE GET_DEL_H:'//&
                  ' W-MOMENTUM CELL',IJK,' IS NOT A CUT CELL'//&
                  ' MFiX will exit now.'
               call log_error()
            ENDIF
  ...
  SUBROUTINE GET_DEL_H_DES(IJK,TYPE_OF_CELL,X0,Y0,Z0,Del_H,Nx,Ny,Nz, allow_neg_dist)
      ...
      SELECT CASE (TYPE_OF_CELL)
         CASE('SCALAR')
            ...
            IF(.NOT.CUT_CELL_AT(IJK)) THEN
               WRITE(ERR_MSG,*)' ERROR IN SUBROUTINE GET_DEL_H_DES:'//&
                  ' SCALAR CELL',IJK,' IS NOT A CUT CELL'//&
                  ' I, J, K =',I_OF(IJK), J_OF(IJK), K_OF(IJK),''//&
                  ' MFiX will exit now.'
               call log_error()
            ENDIF
...
!======================================================================
        ...
        IF(N_CUT_FACE_NODES < 3) THEN
           WRITE(ERR_MSG,*)' ERROR IN SUBROUTINE STORE_CUT_FACE_INFO:'//&
              ' CUT FACE HAS LESS THAN 3 NODES.'//&
              ' MFIX WILL EXIT NOW.'
           call log_error()
        END IF
...
!======================================================================
      ...
      SELECT CASE (TYPE_OF_CELL)
         ...
         CASE DEFAULT
            WRITE(ERR_MSG,*)'SUBROUTINE: STORE_CUT_FACE_INFO'//&
               'UNKNOWN TYPE OF CELL:',TYPE_OF_CELL//&
               'ACCEPTABLE TYPES ARE:'//&
               'SCALAR'//&
               'U_MOMENTUM'//&
...
! Count the number of cut cells
      ...
      DO IJK = IJKSTART3, IJKEND3
         IF(INTERIOR_CELL_AT(IJK).AND.(CUT_CELL_AT(IJK).OR.SMALL_CELL_AT(IJK))) THEN
...
! i.e. the cumulative sum at a given processor.
!======================================================================
...
!  This is done in three passes, in order of likelihood to find a wall:
...
!  explicitly through all cut cells.
