...
MODULE GET_ALPHA_MOD

   USE bc
   USE compar, ONLY: iend1, jend1, kend1, IJKSTART3, IJKEND3
   USE cutcell
   USE error_manager
   USE functions, ONLY: FUNIJK
   USE geometry, ONLY: DO_K, NO_K
   USE geometry, ONLY: axy, axy_w, flag_t
   USE geometry, ONLY: axz, axz_v, flag_n
   USE geometry, ONLY: ayz, ayz_u, flag_e
   USE get_connectivity_mod, only: get_connectivity, get_cell_node_coordinates
   USE get_delh_mod, only: get_del_h
...
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^C
  SUBROUTINE GET_3D_ALPHA_U_CUT_CELL
      ...
      WRITE(ERR_MSG,10)'COMPUTING INTERPOLATION FACTORS IN U-MOMENTUM CELLS...'
      CALL LOG_STATUS()
10    FORMAT(1X,A)
...
!======================================================================
            ...
            IF(BCT ==CG_NSW.OR.BCT ==CG_PSW) THEN
               ...
               IF(DELH_U(IJK)<ZERO.AND.(.NOT.WALL_U_AT(IJK))) THEN
                  WRITE(ERR_MSG,*) 'NEGATIVE DELH-U AT XYZ=', X_U(IJK),Y_U(IJK),Z_U(IJK),''//&
                     'DELH_U=', DELH_U(IJK),''//&
                     'AYZ=', AYZ(IJK),''//&
                     'MFIX WILL EXIT NOW.'
                  CALL LOG_ERROR()
...
! Xe has not moved based on definition of Theta_Ue
               ...
               IF(ALPHA_Ue_c(IJK)<ZERO) THEN
                 WRITE(ERR_MSG,*) 'NEGATIVE ALPHA_Ue_c at IJK=',IJK,''//&
                    'MFIX WILL EXIT NOW.'
                 CALL LOG_ERROR()
               ENDIF
            ...
            IF (  CUT_U_CELL_AT(IJK)  )  THEN
               ...
               SELECT CASE (PG_OPTION)
                  ...
                  CASE DEFAULT
                     ...
                     WRITE(ERR_MSG,*)'Invalid pressure gradient option PG_OPTION:', PG_OPTION,''//&
                        'Valid PG_OPTION values: 0, 1, 2'
                     call log_error()
               ...
               IF (BLOCKED_CELL_AT(IJK).OR.BLOCKED_CELL_AT(IPJK)) THEN
                  IF(.NOT.WALL_U_AT(IJK)) THEN
                     ...
                     IF(PRINT_WARNINGS) THEN
                        WRITE(ERR_MSG,*) 'WARNING: ONLY ONE PRESSURE NODE DETECTED IN U-MOMENTUM CELL IJK =',IJK,''//&
                           'RESETTING U-MOMENTUM CELL AS U_WALL CELL.'
                     ENDIF
...
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^C
  SUBROUTINE GET_3D_ALPHA_V_CUT_CELL
      ...
      WRITE(ERR_MSG,10)'COMPUTING INTERPOLATION FACTORS IN V-MOMENTUM CELLS...'
      CALL LOG_STATUS()
10    FORMAT(1X,A)
...
!======================================================================
            ...
            IF(BCT ==CG_NSW.OR.BCT ==CG_PSW) THEN
               ...
               IF(DELH_V(IJK)<ZERO.AND.(.NOT.WALL_V_AT(IJK))) THEN
                 WRITE(ERR_MSG,*) 'NEGATIVE DELH-V AT XYZ=', X_V(IJK),Y_V(IJK),Z_V(IJK),''//&
                    'DELH_V=', DELH_V(IJK),''//&
                    'AYZ=', AXZ(IJK),''//&
                    'MFIX WILL EXIT NOW.'
                 CALL LOG_ERROR()
...
! Yn has not moved based on definition of Theta_V
               ...
               IF(ALPHA_Vn_c(IJK)<ZERO) THEN
                 WRITE(ERR_MSG,*) 'NEGATIVE ALPHA_Vn_c at IJK=',IJK,''//&
                    'MFIX WILL EXIT NOW.'
                 CALL LOG_ERROR()
               ENDIF
            ...
            IF (  CUT_V_CELL_AT(IJK)  )  THEN
               ...
               IF (BLOCKED_CELL_AT(IJK).OR.BLOCKED_CELL_AT(IJPK)) THEN
                  IF(.NOT.WALL_V_AT(IJK)) THEN
                     ...
                     FLAG_N(IJK) = 0
                     IF(PRINT_WARNINGS) THEN
                        WRITE(ERR_MSG,*) 'WARNING: ONLY ONE PRESSURE NODE DETECTED IN V-MOMENTUM CELL IJK =',IJK,''//&
                           'RESETTING U-MOMENTUM CELL AS V_WALL CELL.'
                     ENDIF
!                     write(*,*) 'MFiX will exit now.'
!                     call log_error()
                  ENDIF
...
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^C
  SUBROUTINE GET_3D_ALPHA_W_CUT_CELL
      ...
      WRITE(ERR_MSG,10)'COMPUTING INTERPOLATION FACTORS IN W-MOMENTUM CELLS...'
      CALL LOG_STATUS()
10    FORMAT(1X,A)
...
!======================================================================
            ...
            IF(BCT ==CG_NSW.OR.BCT ==CG_PSW) THEN
               ...
               IF(DELH_W(IJK)<ZERO.AND.(.NOT.WALL_W_AT(IJK))) THEN
                  WRITE(ERR_MSG,*) 'NEGATIVE DELH-W AT XYZ=', X_W(IJK),Y_W(IJK),Z_W(IJK),''//&
                     'DELH_W=', DELH_W(IJK),''//&
                     'AXY=', AXY(IJK),''//&
                     'MFIX WILL EXIT NOW.'
                  CALL LOG_ERROR()
...
! Zt has not moved based on definition of Theta_Ut
               ...
               IF(ALPHA_Wt_c(IJK)<ZERO) THEN
                  WRITE(ERR_MSG,*) 'NEGATIVE ALPHA_Wt_c at IJK=',IJK,''//&
                     'MFIX WILL EXIT NOW.'
                  CALL LOG_ERROR()
               ENDIF
            ...
            IF (  CUT_W_CELL_AT(IJK)  )  THEN
               ...
               IF (BLOCKED_CELL_AT(IJK).OR.BLOCKED_CELL_AT(IJKP)) THEN
                  IF(.NOT.WALL_W_AT(IJK)) THEN
                     ...
                     FLAG_T(IJK) = 0
                     IF(PRINT_WARNINGS) THEN
                        WRITE(ERR_MSG,*) 'WARNING: ONLY ONE PRESSURE NODE DETECTED IN W-MOMENTUM CELL IJK =',IJK,''//&
                           'RESETTING U-MOMENTUM CELL AS W_WALL CELL.'
                     ENDIF
!                     write(*,*) 'MFiX will exit now.'
!                     call log_error()
                  ENDIF
