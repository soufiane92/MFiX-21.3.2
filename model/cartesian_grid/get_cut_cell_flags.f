...
MODULE GET_CUT_CELL_FLAGS_MOD

   USE bc
   USE compar, only: iend1, iend2, iend3, istart1, istart2, istart3
   USE compar, only: jend1, jend2, jend3, jstart1, jstart2, jstart3
   USE compar, only: kend1, kend2, kend3, kstart1, kstart2, kstart3
   USE compar, only: pe_io, mype, ijkstart3, ijkend3
   USE cutcell
   USE dmp_cartesian, only: send_receive_1d_logical
   USE error_manager
   USE eval_f_mod, only: eval_f
   USE functions, only: funijk, fluid_at, is_on_mype_wobnd
   ...
   USE geometry, ONLY: dx, ayz, axy_u, ayz_u, axz_u, vol_u, imax1, imin1, flag_n, do_i
   USE geometry, ONLY: dy, axz, axy_v, axz_v, ayz_v, vol_v, jmax1, jmin1, flag_e, do_j
   USE geometry, ONLY: dz, axy, axy_w, axz_w, ayz_w, vol_w, kmax1, kmin1, flag_t, do_k
   USE geometry, ONLY: flag, vol, no_k
   USE get_connectivity_mod, only: get_connectivity, get_cell_node_coordinates
   USE get_cut_cell_volume_and_areas_mod, only: get_cut_cell_volume_and_areas
   USE get_delh_mod, only: get_del_h_des
   USE get_stl_data_mod, only: eval_stl_fct_at
   USE indices, only: i_of, j_of, k_of
   USE intersect_mod, ONLY: cad_intersect, clean_intersect, clean_intersect_scalar, intersect
   ...
   USE write_progress_bar_mod, only: write_progress_bar
   USE run, only:ppo, generate_mesh
...
!======================================================================
      ...
      CALL INITIALIZE_VEL_NODES

      IF(.NOT.GENERATE_MESH) RETURN

       NUMBER_OF_NODES = 0

      CALL GET_POTENTIAL_CUT_CELLS

      WRITE(ERR_MSG,10)'INTERSECTING GEOMETRY WITH SCALAR CELLS...'
      CALL LOG_STATUS()
10    FORMAT(1X,A)
...
!        NUMBER_OF_NODES = 0
      ...
      IF(SIZE(X_NEW_POINT)<DIMENSION_MAX_CUT_CELL) CALL REAL_GROW(X_NEW_POINT,DIMENSION_MAX_CUT_CELL)
      IF(SIZE(Y_NEW_POINT)<DIMENSION_MAX_CUT_CELL) CALL REAL_GROW(Y_NEW_POINT,DIMENSION_MAX_CUT_CELL)
      IF(SIZE(Z_NEW_POINT)<DIMENSION_MAX_CUT_CELL) CALL REAL_GROW(Z_NEW_POINT,DIMENSION_MAX_CUT_CELL)
...
!JFD         IF(POTENTIAL_CUT_CELL_AT(IJK))  THEN
         IF(.TRUE.)  THEN
!======================================================================
...
! Initialize connectivity as that of a standard cell
            ...
            IF(NUMBER_OF_NEW_POINTS>DIMENSION_MAX_CUT_CELL-20) THEN
               DIMENSION_MAX_CUT_CELL = 2*DIMENSION_MAX_CUT_CELL
               CALL REAL_GROW(X_NEW_POINT,DIMENSION_MAX_CUT_CELL)
               CALL REAL_GROW(Y_NEW_POINT,DIMENSION_MAX_CUT_CELL)
               CALL REAL_GROW(Z_NEW_POINT,DIMENSION_MAX_CUT_CELL)
            ENDIF

            TOTAL_NUMBER_OF_INTERSECTIONS = 0 
...
!               IF(TOTAL_NUMBER_OF_INTERSECTIONS==-1) F_NODE(0) = -ONE
               ...
               ! JFD READ FROM MSH FILE BC_ID(IJK) = 0
...
! Keep a copy of F_AT, and SNAP for the scalar cells. Needed for the Mesh file
      F_AT_SCALAR = F_AT
      SNAP_SCALAR = SNAP
...
!                  print*,'comparing:',IJK,' and',IJK_NB
                  ...
                  DO NODE = 1,NUMBER_OF_NODES(IJK)
                     ...
                     ENDIF ! node belongs to the cut-face
                  ENDDO ! NODE

               ENDIF ! cut-cell at NB
            ENDDO   ! NB

         ENDIF  !cut-cell
      ENDDO  ! IJK loop

!      print*,'Removing duplicate nodes...Done'
...
!             print*,'NODE,VOL=',IJK,Ovol_around_node(IJK)
      ...
      IF(TOT_VOL_CELLS>ZERO) THEN
         IF(DABS(ONE-DABS(TOT_VOL_NODE/TOT_VOL_CELLS))>1.0D-6) THEN
            WRITE(ERR_MSG,*)'==========================================================='//&
               ...
               'The two volumes above should be the same.',''//&
               '==========================================================='
            CALL LOG_ERROR()
         ENDIF
      ENDIF
      ...
      SUBROUTINE SET_3D_CUT_CELL_FLAGS_ADD
      ...
      IF(GENERATE_MESH) RETURN

      IF(MyPE == PE_IO) THEN
         WRITE(*,10)'ADDITIONAL SCALAR CELL SETTINGS...'
      ENDIF
10    FORMAT(1X,A)
...
!       ENDIF
      ...
      RETURN
      END SUBROUTINE SET_3D_CUT_CELL_FLAGS_ADD

!vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvC
!                                                                      C
!  Module name: INITIALIZE_VEL_NODES                                   C
!  Purpose: Initialize velocity node locations                         C
!                                                                      C
!  Author: Jeff Dietiker                              Date: 21-Feb-08  C
!  Reviewer:                                          Date:            C
!                                                                      C
...
!                                                                      C
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^C
  SUBROUTINE INITIALIZE_VEL_NODES
      ...
      IF(MyPE == PE_IO.AND.GENERATE_MESH) THEN
         WRITE(*,10)'INITIALIZING VELOCITY NODES...'
      ENDIF
10    FORMAT(1X,A)
...
!======================================================================
!  Get coordinates of eight nodes - Scalar cells
!======================================================================
...
!======================================================================
!  Get coordinates of eight nodes - U-momentum cell
!======================================================================
...
!======================================================================
!  Get coordinates of eight nodes - V-Momentum cell
!======================================================================
...
!======================================================================
!  Get coordinates of eight nodes - W-momentum cell
!======================================================================
...
! W-Momentum DX,DY,DZ      
      ...
      RETURN
      END SUBROUTINE INITIALIZE_VEL_NODES
...
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^C
  SUBROUTINE SET_3D_CUT_U_CELL_FLAGS
      ...
      WRITE(ERR_MSG,10)'INTERSECTING GEOMETRY WITH U-MOMENTUM CELLS...'
      CALL LOG_STATUS()
10    FORMAT(1X,A)
!======================================================================
!  Intersection between quadric Grid
...
!               IF(POTENTIAL_CUT_CELL_AT(IJK))  CALL CLEAN_INTERSECT(IJK,'U_MOMENTUM',Xn_U_int(IJK),Ye_U_int(IJK),Zt_U_int(IJK))
      ...
      IF(SIZE(X_NEW_U_POINT)<DIMENSION_MAX_CUT_CELL) CALL REAL_GROW(X_NEW_U_POINT,DIMENSION_MAX_CUT_CELL)
      IF(SIZE(Y_NEW_U_POINT)<DIMENSION_MAX_CUT_CELL) CALL REAL_GROW(Y_NEW_U_POINT,DIMENSION_MAX_CUT_CELL)
      IF(SIZE(Z_NEW_U_POINT)<DIMENSION_MAX_CUT_CELL) CALL REAL_GROW(Z_NEW_U_POINT,DIMENSION_MAX_CUT_CELL)
...
!======================================================================
            ...
            IF(NUMBER_OF_NEW_U_POINTS>DIMENSION_MAX_CUT_CELL-20) THEN
               DIMENSION_MAX_CUT_CELL = 2*DIMENSION_MAX_CUT_CELL
               CALL REAL_GROW(X_NEW_U_POINT,DIMENSION_MAX_CUT_CELL)
               CALL REAL_GROW(Y_NEW_U_POINT,DIMENSION_MAX_CUT_CELL)
               CALL REAL_GROW(Z_NEW_U_POINT,DIMENSION_MAX_CUT_CELL)
            ENDIF
            ...
            IF(TOTAL_NUMBER_OF_INTERSECTIONS < MIN_INTERSECTIONS ) THEN   ! Not a cut cell
               ...
               IF(TOTAL_NUMBER_OF_INTERSECTIONS==-1) F_NODE(0) = -ONE
               ! JFD read from MSH file BC_U_ID(IJK) = 0
...
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^C
  SUBROUTINE SET_3D_CUT_V_CELL_FLAGS
      ...
      WRITE(ERR_MSG,*)'INTERSECTING GEOMETRY WITH V-MOMENTUM CELLS...'
      CALL LOG_STATUS()
!======================================================================
!  Intersection between quadric Grid
!======================================================================
...
!                IF(POTENTIAL_CUT_CELL_AT(IJK)) CALL CLEAN_INTERSECT(IJK,'V_MOMENTUM',Xn_V_int(IJK),Ye_V_int(IJK),Zt_V_int(IJK))
      ...
      IF(SIZE(X_NEW_V_POINT)<DIMENSION_MAX_CUT_CELL) CALL REAL_GROW(X_NEW_V_POINT,DIMENSION_MAX_CUT_CELL)
      IF(SIZE(Y_NEW_V_POINT)<DIMENSION_MAX_CUT_CELL) CALL REAL_GROW(Y_NEW_V_POINT,DIMENSION_MAX_CUT_CELL)
      IF(SIZE(Z_NEW_V_POINT)<DIMENSION_MAX_CUT_CELL) CALL REAL_GROW(Z_NEW_V_POINT,DIMENSION_MAX_CUT_CELL)
...
!======================================================================
            ...
            IF(NUMBER_OF_NEW_V_POINTS>DIMENSION_MAX_CUT_CELL-20) THEN
               DIMENSION_MAX_CUT_CELL = 2*DIMENSION_MAX_CUT_CELL
               CALL REAL_GROW(X_NEW_V_POINT,DIMENSION_MAX_CUT_CELL)
               CALL REAL_GROW(Y_NEW_V_POINT,DIMENSION_MAX_CUT_CELL)
               CALL REAL_GROW(Z_NEW_V_POINT,DIMENSION_MAX_CUT_CELL)
            ENDIF
            ...
            IF(TOTAL_NUMBER_OF_INTERSECTIONS < MIN_INTERSECTIONS ) THEN   ! Not a cut cell
               ...
               IF(TOTAL_NUMBER_OF_INTERSECTIONS==-1) F_NODE(0) = -ONE
               ! JFD read from MSH file BC_V_ID(IJK) = 0
               ...
               ELSE
                  ...
                  CUT_V_CELL_AT(IJK)      = .FALSE.          ! Blocked fluid cell
            ...
            ELSE IF(TOTAL_NUMBER_OF_INTERSECTIONS > MAX_INTERSECTIONS ) THEN
               ...
               IF(PRINT_WARNINGS) THEN
                  WRITE(ERR_MSG,*) 'TOO MANY INTERSECTIONS FOUND IN V-CELL IJK = ',IJK,''//&
                     'MAXIMUM NUMBER OF INTERSECTIONS = ',MAX_INTERSECTIONS,''//&
                     'TOTAL NUMBER OF INTERSECTIONS = ',TOTAL_NUMBER_OF_INTERSECTIONS,''//&
                     'REMOVING V-CELL FROM COMPUTATION.'
!                  WRITE(*,*) 'MFIX WILL EXIT NOW.'
!                  CALL LOG_ERROR()
...
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^C
  SUBROUTINE SET_3D_CUT_W_CELL_FLAGS
      ...
      WRITE(ERR_MSG,10)'INTERSECTING GEOMETRY WITH W-MOMENTUM CELLS...'
      CALL LOG_STATUS()
10    FORMAT(1X,A)
!======================================================================
!  Intersection between quadric Grid
...
!               IF(POTENTIAL_CUT_CELL_AT(IJK)) CALL CLEAN_INTERSECT(IJK,'W_MOMENTUM',Xn_W_int(IJK),Ye_W_int(IJK),Zt_W_int(IJK))
      ...
      IF(SIZE(X_NEW_W_POINT)<DIMENSION_MAX_CUT_CELL) CALL REAL_GROW(X_NEW_W_POINT,DIMENSION_MAX_CUT_CELL)
      IF(SIZE(Y_NEW_W_POINT)<DIMENSION_MAX_CUT_CELL) CALL REAL_GROW(Y_NEW_W_POINT,DIMENSION_MAX_CUT_CELL)
      IF(SIZE(Z_NEW_W_POINT)<DIMENSION_MAX_CUT_CELL) CALL REAL_GROW(Z_NEW_W_POINT,DIMENSION_MAX_CUT_CELL)
...
!======================================================================
            ...
            IF(NUMBER_OF_NEW_W_POINTS>DIMENSION_MAX_CUT_CELL-20) THEN
               DIMENSION_MAX_CUT_CELL = 2*DIMENSION_MAX_CUT_CELL
               CALL REAL_GROW(X_NEW_W_POINT,DIMENSION_MAX_CUT_CELL)
               CALL REAL_GROW(Y_NEW_W_POINT,DIMENSION_MAX_CUT_CELL)
               CALL REAL_GROW(Z_NEW_W_POINT,DIMENSION_MAX_CUT_CELL)
            ENDIF
            ...
            IF(TOTAL_NUMBER_OF_INTERSECTIONS < 3 ) THEN   ! Not a cut cell
               ...
               IF(TOTAL_NUMBER_OF_INTERSECTIONS==-1) F_NODE(0) = -ONE
               ! JFD read from MSH file BC_W_ID(IJK) = 0
            ...
            ELSE IF(TOTAL_NUMBER_OF_INTERSECTIONS > 6 ) THEN
               ...
               IF(PRINT_WARNINGS) THEN
                  WRITE(ERR_MSG,*) 'TOO MANY INTERSECTIONS FOUND IN W-CELL IJK = ',IJK,''//&
                     'MAXIMUM NUMBER OF INTERSECTIONS = ',6,''//&
                     'TOTAL NUMBER OF INTERSECTIONS = ',TOTAL_NUMBER_OF_INTERSECTIONS,''//&
                     'REMOVING W-CELL FROM COMPUTATION.'
!                  WRITE(*,*) 'MFIX WILL EXIT NOW.'
!                  CALL LOG_ERROR()
...
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^C
  SUBROUTINE SET_3D_CUT_CELL_TREATMENT_FLAGS
      ...
      WRITE(ERR_MSG,*)'SETTING CUT CELL TREATMENT FLAGS...'
      CALL LOG_STATUS()
...
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^C
  SUBROUTINE SET_GHOST_CELL_FLAGS
      ...
      USE functions, ONLY: WEST_OF, EAST_OF, SOUTH_OF, NORTH_OF, BOTTOM_OF, TOP_OF
      USE functions, ONLY: IM_OF, IP_OF, JM_OF, JP_OF, KM_OF, KP_OF
      USE geometry, ONLY: imax1, imin1, jmax1, jmin1, kmax1, kmin1, vol, vol_u, vol_v, vol_w, flag, flag_e, flag_n, flag_t
      USE geometry, ONLY: axy, axz, ayz, ayz_u, ayz_v, ayz_w, axy_u, axy_v, axy_w, axz_u, axz_v, axz_w, do_k
...
!     JFD: I am turning this subroutine off. This looks like overkill and may not be necessary.
         ...
         DO I23 = IEND2,IEND3
            DO K = KSTART3,KEND3
               DO J = JSTART3, JEND3
                  ...
                  IF(BLOCKED_CELL_AT(IPJK)) THEN
                     STANDARD_CELL_AT(IPJK) = .FALSE.
                     CUT_CELL_AT(IPJK)      = .FALSE.
                     FLAG(IPJK)             = 100
                     AXY(IPJK)              = ZERO
                     AXZ(IPJK)              = ZERO
                     AYZ(IPJK)              = ZERO
         ...
         DO I23 = ISTART3,ISTART2
            DO K = KSTART3,KEND3
               DO J = JSTART3, JEND3
                  ...
                  IF(BLOCKED_CELL_AT(IMJK)) THEN
                     STANDARD_CELL_AT(IMJK) = .FALSE.
                     CUT_CELL_AT(IMJK)      = .FALSE.
                     FLAG(IMJK)             = 100
                     AXY(IMJK)              = ZERO
                     AXZ(IMJK)              = ZERO
                     AYZ(IMJK)              = ZERO
         ...
         DO J23 = JEND2,JEND3
            DO K = KSTART3,KEND3
               DO I = ISTART3, IEND3
                  ...
                  IF(BLOCKED_CELL_AT(IJPK)) THEN
                     STANDARD_CELL_AT(IJPK) = .FALSE.
                     CUT_CELL_AT(IJPK)      = .FALSE.
                     FLAG(IJPK)             = 100
                     AXY(IJPK)              = ZERO
                     AXZ(IJPK)              = ZERO
                     AYZ(IJPK)              = ZERO
         ...
         DO J23 = JSTART3,JSTART2
            DO K = KSTART3,KEND3
               DO I = ISTART3, IEND3
                  ...
                  IF(BLOCKED_CELL_AT(IJMK)) THEN
                     STANDARD_CELL_AT(IJMK) = .FALSE.
                     CUT_CELL_AT(IJMK)      = .FALSE.
                     FLAG(IJMK)             = 100
                     AXY(IJMK)              = ZERO
                     AXZ(IJMK)              = ZERO
                     AYZ(IJMK)              = ZERO
            ...
            DO K23=KEND2,KEND3
               DO J = JSTART3,JEND3
                  DO I = ISTART3, IEND3
                     ...
                     IF(BLOCKED_CELL_AT(IJKP)) THEN
                        STANDARD_CELL_AT(IJKP) = .FALSE.
                        CUT_CELL_AT(IJKP)      = .FALSE.
                        FLAG(IJKP)             = 100
                        AXY(IJKP)              = ZERO
                        AXZ(IJKP)              = ZERO
                        AYZ(IJKP)              = ZERO
            ...
            DO K23 = KSTART3,KSTART2
               DO J = JSTART3,JEND3
                  DO I = ISTART3, IEND3
                     ...
                     IF(BLOCKED_CELL_AT(IJKM)) THEN
                        STANDARD_CELL_AT(IJKM) = .FALSE.
                        CUT_CELL_AT(IJKM)      = .FALSE.
                        FLAG(IJKM)             = 100
                        AXY(IJKM)              = ZERO
                        AXZ(IJKM)              = ZERO
                        AYZ(IJKM)              = ZERO
...
!                                                                      C
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^C
  SUBROUTINE SET_GHOST_CELL_FLAGS_OLD
...
!     THE FLAG IS OVERWRITTEN HERE TO ACCOUNT FOR NSW,FSW AND PSW BCs
      ...
      END SUBROUTINE SET_GHOST_CELL_FLAGS_OLD
...
!                                                                      C
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^C
  SUBROUTINE GET_POTENTIAL_CUT_CELLS_OLD
      ...
      DO IJK = IJKSTART3, IJKEND3
         ...
         IF(INTERIOR_CELL_AT(IJK)) THEN
            ...
            IF((FLAG(IJK)>=100).AND.(FLAG(IJK)<=102)) THEN
               ...
               CUT_CELL_AT(IJK)           = .FALSE.          ! Blocked cell = wall cell
               BLOCKED_CELL_AT(IJK)       = .TRUE.
            ENDIF
...
!      call SEND_RECEIVE_1D_LOGICAL(SNAP,2)
   ...
   END SUBROUTINE GET_POTENTIAL_CUT_CELLS_OLD 
...
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^C
  SUBROUTINE GET_POTENTIAL_CUT_CELLS
      ...
      INTEGER :: IJK_NB,NUMBER_OF_POTENTIAL_CUT_CELLS
      DOUBLE PRECISION :: X_OFFSET, Y_OFFSET, Z_OFFSET,TOL_PC

 ! Additional tolerance to identify potential cut cells
      TOL_PC = 0.1D0
...
!      RETURN  ! This subroutine is currently disabled
      ...
      WRITE(ERR_MSG,10)'ESTIMATING POTENTIAL SCALAR CUT CELLS...'
      CALL LOG_STATUS()

10    FORMAT(1X,A)


      POTENTIAL_CUT_CELL_AT = .FALSE.
!      POTENTIAL_CUT_CELL_AT = .TRUE.
!      RETURN
...
! Skip intersection if the STL facet is flush with MFiX domain box.
! This will provide better mesh
         ...
         IF(X2>=X_MIN-DX(ISTART3).AND.X1<=X_MAX+DX(IEND3)) THEN
            DO I = ISTART3, IEND3
               IP = I+1
               IF(XG_E(I)+X_OFFSET*DX(IP)>=X1-(TOL_PC+TOL_SNAP(1))*DX(I)) THEN
                  I1=I
                  EXIT
               ENDIF
            ENDDO
            ...
            DO I = IEND3, ISTART3,-1
               IP = I+1
               IF(XG_E(I)-DX(I)+X_OFFSET*DX(IP)<=X2+(TOL_PC+TOL_SNAP(1))*DX(I)) THEN
                  I2=I
                  EXIT
               ENDIF
            ENDDO
         ...
         IF(Y2>=Y_MIN-DY(JSTART3).AND.Y1<=Y_MAX+DY(JEND3)) THEN
            DO J = JSTART3, JEND3
               JP = J+1
               IF(YG_N(J)+Y_OFFSET*DY(JP)>=Y1-TOL_STL-(TOL_PC+TOL_SNAP(2))*DY(J)) THEN
                  J1=J
                  EXIT
               ENDIF
            ENDDO
            ...
            DO J = JEND3, JSTART3,-1
               JP=J+1
               IF(YG_N(J)-DY(J)+Y_OFFSET*DY(JP)<=Y2+TOL_STL+(TOL_PC+TOL_SNAP(2))*DY(J)) THEN
                  J2=J
                  EXIT
               ENDIF
            ENDDO
         ...
         IF(Z2>=Z_MIN-DZ(KSTART3).AND.Z1<=Z_MAX+DZ(KEND3)) THEN
            DO K = KSTART3, KEND3
               ...
               IF(ZG_T(K)+Z_OFFSET*DZ(KP)>=Z1-TOL_STL-(TOL_PC+TOL_SNAP(3))*DZ(K)) THEN
                  K1=K
                  EXIT
               ENDIF
            ENDDO
            ...
            DO K = KEND3, KSTART3,-1
               KP = K+1
               IF(ZG_T(K)-DZ(K)+Z_OFFSET*DZ(KP)<=Z2+TOL_STL+(TOL_PC+TOL_SNAP(3))*DZ(K)) THEN
                  K2=K
                  EXIT
               ENDIF
            ENDDO
         ...
         DO K=K1,K2
            DO J=J1,J2
               DO I=I1,I2
                  ...
                  ! POTENTIAL_CUT_CELL_AT(IPJK) = .TRUE.
                  ! POTENTIAL_CUT_CELL_AT(IJPK) = .TRUE.
                  ! POTENTIAL_CUT_CELL_AT(IJKP) = .TRUE.
                  !
                  ! POTENTIAL_CUT_CELL_AT(IMJK) = .TRUE.
                  ! POTENTIAL_CUT_CELL_AT(IJMK) = .TRUE.
                  ! POTENTIAL_CUT_CELL_AT(IJKM) = .TRUE.
                  !
                  ! POTENTIAL_CUT_CELL_AT(IJPKP) = .TRUE.
                  ! POTENTIAL_CUT_CELL_AT(IPJKP) = .TRUE.
                  ! POTENTIAL_CUT_CELL_AT(IPJPK) = .TRUE.
               ENDDO  ! I loop
            ENDDO  ! J loop
         ENDDO  ! K loop
...
!RESET      POTENTIAL_CUT_CELL_AT = .TRUE.
...
END MODULE GET_CUT_CELL_FLAGS_MOD
