...
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^C
  SUBROUTINE SET_ODXYZ_U_CUT_CELL
      ...
      IF(NO_K) THEN
         WRITE(ERR_MSG,10)'COMPUTING 1/DX, 1/DY FOR U-MOMENTUM CELLS...'
      ELSE
         WRITE(ERR_MSG,10)'COMPUTING 1/DX, 1/DY, 1/DZ FOR U-MOMENTUM CELLS...'
      ENDIF
      CALL LOG_STATUS()
10    FORMAT(1X,A)
...
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^C
   SUBROUTINE SET_ODXYZ_V_CUT_CELL
      ...
      IF(MyPE == PE_IO) THEN
         IF(NO_K) THEN
            WRITE(ERR_MSG,10)'COMPUTING 1/DX, 1/DY FOR V-MOMENTUM CELLS...'
         ELSE
            WRITE(ERR_MSG,10)'COMPUTING 1/DX, 1/DY, 1/DZ FOR V-MOMENTUM CELLS...'
         ENDIF
         CALL LOG_STATUS()
      ENDIF
10    FORMAT(1X,A)
...
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^C
   SUBROUTINE SET_ODXYZ_W_CUT_CELL
      ...
      WRITE(ERR_MSG,10)'COMPUTING 1/DX, 1/DY, 1/DZ FOR W-MOMENTUM CELLS...'
      CALL LOG_STATUS()
10    FORMAT(1X,A)
