...
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^C
   ...
   SUBROUTINE BC_THETA(M, A_m, B_m)
      ...
      USE functions, only: bottom_of, east_of, fluid_at, north_of, south_of, top_of, west_of, funijk, flow_at, is_on_mype_plus2layers
      USE geometry, only: odx_e, ody_n, odz_t, ox
      USE indices, only: im1, jm1, km1
      USE param, only: bottom, east, north, south, top, west, dimension_m, dimension_3
      USE param1, only: one, zero, half
...
!     term. A more rigorous implementation should account for these    C
...
! should never hit this
         WRITE (ERR_MSG, '(A,A,A)') 'BC_THETA => THETA_HW_CW'//&
