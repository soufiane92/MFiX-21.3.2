...
!  is used in evaluating limiting values of certain granular kinetic   C
!  theory terms                                                        C
!                                                                      C
...
! the term ED_s * (TL-TM) cancels. Therefore, explicitly set
...
! velocity of solid phase L: do not explicitly include
...
! Dissipation associated with divergence of
! velocity of solid phase M: do not explicitly include
! terms that cancel when EDvel_sL is summed with EDvel_sM
...
! this is done to avoid /0 in case c_e = 1.0
            ...
            if(EP_SM> small_number .and. Re_T > 2.0d-4) then ! avoid singularities
...
! theory.
! note that theta_(ijk,m) doesn't contain mass m, so that T/m = theta_m.
...
!---------------------------------------------------------------------//
      ...
      K_phi = DSQRT(0.3d0*phi)/(1.0d0-phi)**3.6d0
      RETURN
      END FUNCTION K_phi
...
!---------------------------------------------------------------------//
      ...
      if (phi > 1d-15) R_d = &
      10d0*phi/(1d0-phi)**3 + (1.0d0 - phi)*(1.0d0 + 3.0d0*DSQRT(phi))
