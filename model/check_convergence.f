...
!      if(abs(errorpercent) > MaxErrorPercent)then
        CALL DETECT_NAN_IN_RESIDUAL(RESID(RESID_P,0),'Pressure correction equation, phase '//trim(ivar('',0)))
        SUM = RESID(RESID_P,0)
!      else
!        SUM = zero
!      endif
...
! add solids correction residual
      CALL DETECT_NAN_IN_RESIDUAL(RESID(RESID_P,1),'Pressure correction equation, phase '//trim(ivar('',1)))
      IF(MMAX > 0) SUM = SUM + RESID(RESID_P,1)

! add continuity equation residuals
      DO M = 0, MMAX
         CALL DETECT_NAN_IN_RESIDUAL(RESID(RESID_RO,M),'Continuity equation, phase '//trim(ivar('',M)))
         SUM = SUM + RESID(RESID_RO,M)
      ENDDO
! add momentum equation residuals
      DO M = 0, MMAX
         CALL DETECT_NAN_IN_RESIDUAL(RESID(RESID_U,M),'U-Momentum equation, phase '//trim(ivar('',M)))
         SUM = SUM + RESID(RESID_U,M)
      ENDDO
      DO M = 0, MMAX
         CALL DETECT_NAN_IN_RESIDUAL(RESID(RESID_V,M),'V-Momentum equation, phase '//trim(ivar('',M)))
         SUM = SUM + RESID(RESID_V,M)
      ENDDO
      IF (DO_K) THEN
         DO M = 0, MMAX
            CALL DETECT_NAN_IN_RESIDUAL(RESID(RESID_W,M),'W-Momentum equation, phase '//trim(ivar('',M)))
...
! sum the granular energy equation residuals
      ...
      IF (GRANULAR_ENERGY) THEN
         DO M = 1, MMAX
            CALL DETECT_NAN_IN_RESIDUAL(RESID(RESID_TH,M),'Granular energy equation, phase '//trim(ivar('',M)))
...
! sum the energy equation residuals
      ...
      IF (ENERGY_EQ) THEN
         DO M = 0, MMAX
            CALL DETECT_NAN_IN_RESIDUAL(RESID(RESID_T,M),'Energy equation, phase '//trim(ivar('',M)))
...
! sum the species equation residuals
      ...
      DO M = 0, MMAX
         IF (SPECIES_EQ(M)) THEN
            DO NN = 1, NMAX(M)
               CALL DETECT_NAN_IN_RESIDUAL(RESID(RESID_X+(NN-1),M),'Species equation, phase,species '//trim(ivar('',M,NN)))
...
!      call global_all_sum(SUM_X)
      ...
      IF(NScalar > 0) CALL DETECT_NAN_IN_RESIDUAL(RESID(RESID_sc,0),'Scalar equation')
      IF(K_EPSILON) CALL DETECT_NAN_IN_RESIDUAL(RESID(RESID_ke,0),'K-Epsilon equation')
...
! Require at least two iterations.
      IF(NIT == 1) THEN
         IF(SUM + SUM_T + RESID(RESID_sc,0)+ SUM_X + RESID(RESID_ke,0) + SUM_Th <= TOL_SUM_RESID_ABS) THEN
            ! Declare convergence if the sum of all residuals is below
            ! TOL_SUM_RESID_ABS (absolute tolerance). Helps run faster at steady
            ! state
            MUSTIT = 0
         ELSE
            MUSTIT = 1
...
! only check velocity if any of the momentum equations are solved
   ...
   SUBROUTINE DETECT_NAN_IN_RESIDUAL(RESIDUAL,label)
   USE error_manager
   ...
   IF(mfix_isnan(RESIDUAL)) THEN
      WRITE(ERR_MSG,1000) trim(label)
      CALL LOG_ERROR()
   ENDIF

 1000 FORMAT('Error 1000: NaN detected in Residual:',A)

   RETURN


   END SUBROUTINE DETECT_NAN_IN_RESIDUAL


END MODULE CHECK_CONVERGENCE_MOD
