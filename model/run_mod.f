...
! Version of input file(s)
      CHARACTER(LEN=80) :: PROJECT_VERSION
...
! counter to keep track of how many auto_restart were performed
      INTEGER :: ITER_RESTART

! version.release of software
#ifdef MFIX_VERSION
      CHARACTER(LEN=10) :: ID_VERSION = MFIX_VERSION
#else
#error "Error in build scripts; MFIX_VERSION undefined"
#endif
...
! Parameter used with the DIFELICE_GANSER drag model
      DOUBLE PRECISION :: SPHERICITY_DG ! Spericity
      DOUBLE PRECISION :: REF_LENGTH_DG ! Reference length (bed diameter)
...
! If .TRUE. use Scheffer frictional stress (default set to .TRUE.)
      LOGICAL :: SCHAEFFER
! If .TRUE. use Priceton (Srivastava) frictional stress (default set to .FALSE.)
      LOGICAL :: PRINCETON
! If .TRUE. use Guo-Boyce frictional stress (default set to .FALSE.)
      LOGICAL :: GUO_BOYCE
...
! Run pre-processing only (mesh generation) and exit
      LOGICAL :: PPO

 ! NREL_CU_OPT sorting,swappping routines
      INTEGER :: optflag1 = 0

! Generate the mesh and exit if PPO is true
! or run the simulation if PPO is false

      LOGICAL :: GENERATE_MESH

! Flag to know if the mesh file exists in the project directory
      LOGICAL :: MESH_FILE_PRESENT
