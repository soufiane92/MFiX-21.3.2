...
! Stage 1: Free fall
            if(lTime < time_c(lc1)) then
               aStage = 1
               aPos_Y = y_s1(lc1, h0(lc1), lGrav, lTime)
               aVel_Y = dydt_s1(lc1, lGrav, lTime)
! Stage 2: Contact
            elseif( lTime < time_r(lc1)) then
               aStage = 2
               aPos_Y = y_s2(lc1, h0(lc1), lRad(lc1), b_r(lc1),        &
                  w0_r(lc1), lGrav, lTime)
               aVel_Y = dydt_s2(lc1, h0(lc1), lRad(lc1), b_r(lc1),     &
                  w0_r(lc1), lGrav, lTime)
! Stage 3: Rebound
            else
               aStage = 3
               aPos_Y = y_s3(lc1, lRad(lc1), lGrav, lTime)
               aVel_Y = dydt_s3(lc1, lGrav, lTime)
            endif
