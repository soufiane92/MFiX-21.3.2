MODULE SOURCE_U_S_MOD

   use bc
   use bodyforce, only: bfx_s
   use calc_gr_boundary, only: calc_grbdry
   use cg_source_u_s_mod, only: cg_source_u_s, cg_source_u_s_bc
   use compar
   use constant
   use cutcell
   use drag
   ...
   use mms, only: use_mms, mms_u_s_src
   use param, only: dimension_3, dimension_m, north, south, east, west, top, bottom
   use param1, only: one, small_number, zero, half, undefined
   use physprop, only: mmax, cv, smax, close_packed
   use ps
   use run
   use rxns
